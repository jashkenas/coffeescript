...
(function() {
  ...
  var FILE_EXTENSIONS, Lexer, SourceMap, base64encode, checkShebangLine, compile, formatSourcePosition, getSourceMap, helpers, lexer, nodeSourceMapsSupportEnabled, packageJson, parser, ref, registerCompiled, sourceMaps, sources, withPrettyErrors,
    indexOf = [].indexOf;
  ...
  nodeSourceMapsSupportEnabled = (typeof process !== "undefined" && process !== null ? process.execArgv.includes('--enable-source-maps') : void 0) || (typeof process !== "undefined" && process !== null ? (ref = process.env.NODE_OPTIONS) != null ? ref.includes('--enable-source-maps') : void 0 : void 0);
  ...
  exports.compile = compile = withPrettyErrors(function(code, options = {}) {
    var ast, currentColumn, currentLine, encoded, filename, fragment, fragments, generateSourceMap, header, i, j, js, len, len1, map, newLines, nodes, range, ref1, ref2, sourceCodeLastLine, sourceCodeNumberOfLines, sourceMapDataURI, sourceURL, token, tokens, transpiler, transpilerOptions, transpilerOutput, v3SourceMap;
    // Clone `options`, to avoid mutating the `options` object passed in.
    options = Object.assign({}, options);
    // Always generate a source map if no filename is passed in, since without a
    // a filename we have no way to retrieve this source later in the event that
    // we need to recompile it to get a source map for `prepareStackTrace`.
    generateSourceMap = options.sourceMap || options.inlineMap || nodeSourceMapsSupportEnabled || (options.filename == null);
    ...
    if (options.inlineMap || nodeSourceMapsSupportEnabled) {
  ...
  if (nodeSourceMapsSupportEnabled || Error.prepareStackTrace) {
    ...
    registerCompiled = function(filename, source, sourcemap) {
    ...
    getSourceMap = function(filename, line, column) {
      var answer, i, map, ref1, ref2, sourceLocation;
      if (!(filename === '<anonymous>' || (ref1 = filename.slice(filename.lastIndexOf('.')), indexOf.call(FILE_EXTENSIONS, ref1) >= 0))) {
  ...
  checkShebangLine = function(file, input) {
    var args, firstLine, ref1, rest;
    firstLine = input.split(/$/m)[0];
    rest = firstLine != null ? firstLine.match(/^#!\s*([^\s]+\s*)(.*)/) : void 0;
    args = rest != null ? (ref1 = rest[2]) != null ? ref1.split(/\s/).filter(function(s) {
      return s !== '';
    }) : void 0 : void 0;
