...
(function() {
  ...
  CSX_IDENTIFIER = /^(?![\d<])((?:(?!\s)[\-$\w\x7f-\uffff])+(?:\s*:\s*(?:(?!\s)[\-$\w\x7f-\uffff])+|(?:\s*\.\s*(?:(?!\s)[\-$\w\x7f-\uffff])+)+)?)/; // Must not start with `<`.
  // Like `IDENTIFIER`, but includes `-`s
  // JSXNamespacedName
  // Like identifier above
  // JSXMemberExpression
  // Like identifier above
  ...
  CSX_ATTRIBUTE = /^(?!\d)((?:(?!\s)[\-$\w\x7f-\uffff])+(?:\s*:\s*(?:(?!\s)[\-$\w\x7f-\uffff])+)?)([^\S]*=(?!=))?/; // Like `IDENTIFIER`, but includes `-`s.
  // JSXNamespacedName
  // Like identifier above
