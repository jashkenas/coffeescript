...
// everything has been parsed and the JavaScript code generated.
    commentToken() {
      var chunk, comment, commentAttachment, content, here, match, matchIllegal, newLine, prev;
...
// Does this comment follow code on the same line?
      ...
      if (here) {
        ...
        chunk = this.chunk.replace(`###${here}###`, '');
        chunk = this.chunk.replace(/^\n+/, '');
...
// this comment to; and follow with a newline.
        commentAttachment.newLine = true;
        this.lineToken(this.chunk.slice(comment.length));
        this.token('JS', '');
        this.tokens[0].comments = [commentAttachment];
        this.newlineToken(0);
      } else {
        if (prev.comments) {
...
// String-matching-regexes.
  ...
  INSIDE_CSX = /^(?:[^\{<])*/; // Similar to `HEREDOC_DOUBLE` but there is no escaping.
