...
(function() {
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    numberToken() {
      var base, j, len, lexedLength, match, nProp, num, number, numberValue, prev, ref, ref1, s, tag, x;
      ...
      prev = this.prev();
      if (prev && !prev.spaced) {
        if (ref = prev[0], indexOf.call([...CALLABLE, '?.', '::', '?::'], ref) >= 0) {
          s = 0;
          nProp = number;
          if (nProp[0] === ".") {
            this.token('.', '.', s++, 1);
          ...
          ref1 = nProp.split(".");
          for (x = j = 0, len = ref1.length; j < len; x = ++j) {
            num = ref1[x];
            if ((x + 1) % 2 === 0) {
              this.token('.', '.', s++, 1);
            }
            this.token('PROPERTY_NUMBER', num, s, num.length);
            s += num.length;
          }
    ...
    // interpolation.
    stringToken() {
      var $, attempt, delimiter, doc, end, heredoc, i, indent, indentRegex, match, prev, quote, ref, ref1, regex, stringProp, token, tokens;
      ...
      $ = tokens.length - 1;
      if (prev && !prev.spaced && ((ref = prev[0]) === '.' || ref === '?.' || ref === '::' || ref === '?::')) {
        stringProp = prev[0];
      } else {
        stringProp = false;
      }
      ...
      if (heredoc) {
        ...
        this.mergeInterpolationTokens(tokens, {delimiter, stringProp}, (value, i) => {
    ...
    mergeInterpolationTokens(tokens, options, fn) {
      var converted, firstEmptyStringIndex, firstIndex, i, j, k, lastToken, len, len1, locationToken, lparen, placeholderToken, plusToken, rparen, sTag, tag, token, tokensToPush, val, value;
      if (tokens.length > 1) {
        sTag = (options != null ? options.stringProp : void 0) ? ['PROPERTY_STRING_START', 'PROPERTY_STRING_END'] : ['STRING_START', 'STRING_END'];
        lparen = this.token(sTag[0], '(', 0, 0);
      }
      ...
      for (i = j = 0, len = tokens.length; j < len; i = ++j) {
        ...
        switch (tag) {
          ...
          case 'NEOSTRING':
            ...
            token[0] = (options != null ? options.stringProp : void 0) && !lparen ? 'PROPERTY_STRING' : 'STRING';
  ...
  CALLABLE = ['IDENTIFIER', 'PROPERTY', 'PROPERTY_NUMBER', 'PROPERTY_STRING', 'PROPERTY_STRING_END', ')', ']', '?', '@', 'THIS', 'SUPER'];
