...
(function() {
  ...
  ({count, starts, compact, repeat, invertLiterate, merge, attachCommentsToNode, locationDataToString, throwSyntaxError, dump} = require('./helpers'));
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    stringToken() {
      ...
      if (heredoc) {
        ...
        this.mergeInterpolationTokens(tokens, {delimiter, quote}, (value, i) => {
    ...
    mergeInterpolationTokens(tokens, options, fn) {
      var $, attachQuote, converted, firstEmptyStringIndex, firstIndex, i, j, k, lastToken, len, len1, locationToken, lparen, markFinalChunk, markInitialChunk, placeholderToken, plusToken, preserveTagged, quote, rparen, tag, tags, token, tokensToPush, val, value;
      ({quote} = options);
      ...
      for (i = j = 0, len = tokens.length; j < len; i = ++j) {
        ...
        switch (tag) {
          ...
          case 'NEOSTRING':
            ...
            token[0] = 'STRING';
            token[1] = preserveTagged(token)(this.makeDelimitedLiteral(converted, options));
            locationToken = token;
            tokensToPush = [token];
        }
