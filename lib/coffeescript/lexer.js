...
(function() {
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    identifierToken() {
      var alias, badSpread, colon, colonOffset, colonToken, csxSpreadMatch, id, idLength, inCSXTag, input, lastBrace, match, offset, poppedToken, prev, prevprev, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, regex, tag, tagToken;
      inCSXTag = this.atCSXTag();
      if (inCSXTag && this.prev()[1] !== ':' && ((ref = this.chunk[0]) === '{' || ref === '"' || ref === "'")) {
        if ((ref1 = this.chunk[0]) === '"' || ref1 === "'") {
          this.error("Unexpected token");
        }
        csxSpreadMatch = /^({\.\.\.\S+})|({\S+\.\.\.})/.exec(this.chunk);
        if (!csxSpreadMatch) {
          if (lastBrace = /^({[^}:\.]+)(:|\})/.exec(this.chunk)) {
            this.error("Unexpected token, expected ...", {
              offset: lastBrace[0].length - 1
            });
          }
          if (badSpread = /^({\S+\.\.\.)|({\.\.\.\S+)[^\}]/.exec(this.chunk)) {
            offset = badSpread[0][1] === '.' ? badSpread[0].length - 1 : badSpread[0].length;
            this.error("Unexpected token, expected }", {offset});
          }
        }
      }
