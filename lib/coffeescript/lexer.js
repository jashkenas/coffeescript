...
(function() {
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    identifierToken() {
      ...
      regex = inCSXTag ? CSX_IDENTIFIER : IDENTIFIER;
    ...
    csxToken() {
      var colon, csxTag, end, firstChar, id, input, match, origin, token, tokens;
      firstChar = this.chunk[0];
      if (firstChar === '<') {
        if (!(match = CSX_IDENTIFIER.exec(this.chunk.slice(1)))) {
          return 0;
        }
        [input, id, colon] = match;
        ...
        if (this.chunk.slice(0, 2) === '/>') {
          ...
          this.token('CALL_END', ')', end, end + csxTag.name.length + 1);
    ...
    matchWithInterpolations(regex, delimiter, closingDelimiter, interpolators) {
      ...
      if (str.slice(0, closingDelimiter.length) !== closingDelimiter) {
        this.error(`missing ${closingDelimiter}`, {
          length: closingDelimiter.length
        });
      }
