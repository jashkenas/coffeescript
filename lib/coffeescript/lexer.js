...
(function() {
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    replaceUnicodeCodePointEscapes(str, options) {
      var before, chunks, chunksIndex, codePointDecimal, codePointHex, lastEndOffset, length, match, offset, ref, ref1, shouldReplace, start;
      shouldReplace = (ref = (ref1 = options.flags) != null ? ref1.indexOf('u') : void 0) != null ? ref : -1 > -1;
      chunks = [];
      chunksIndex = lastEndOffset = 0;
      while (match = UNICODE_CODE_POINT_ESCAPE.exec(str)) {
        match[0], before = match[1], codePointHex = match[2];
        ({
          index: offset
        } = match);
        ...
        if (shouldReplace) {
          chunks[chunksIndex++] = str.slice(lastEndOffset, start);
          chunks[chunksIndex++] = this.unicodeCodePointToUnicodeEscapes(codePointDecimal);
        }
      ...
      chunks[chunksIndex] = str.slice(lastEndOffset, +(str.length - 1) + 1 || 9e9);
      return chunks.join('');
    }
