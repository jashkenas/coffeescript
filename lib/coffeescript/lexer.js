...
(function() {
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    mergeInterpolationTokens(tokens, options, fn) {
      var attachQuote, converted, firstEmptyStringIndex, firstIndex, i, j, k, lastToken, len, len1, locationToken, lparen, placeholderToken, plusToken, preserveQuote, quote, rparen, tag, token, tokensToPush, val, value;
      ({quote} = options);
      attachQuote = function(token) {
        token[1] = new String(token[1]);
        return token[1].quote = quote;
      };
      preserveQuote = function(token) {
      ...
      for (i = j = 0, len = tokens.length; j < len; i = ++j) {
        ...
        switch (tag) {
          ...
          case 'NEOSTRING':
            ...
            token[0] = 'STRING';
            token[1] = preserveQuote(token)(this.makeDelimitedLiteral(converted, options));
            locationToken = token;
            tokensToPush = [token];
        }
