...
(function() {
  ...
  ({count, starts, compact, repeat, invertLiterate, merge, attachCommentsToNode, locationDataToString, throwSyntaxError, dump, makeDelimitedLiteral} = require('./helpers'));
  ...
  exports.Lexer = Lexer = (function() {
    class Lexer {
      ...
      mergeInterpolationTokens(tokens, options, fn) {
        var $, attachDouble, attachQuote, converted, double, firstEmptyStringIndex, firstIndex, i, indent, j, k, lastToken, len, len1, locationToken, lparen, markFinalChunk, markIndent, markInitialChunk, placeholderToken, plusToken, preserveTagged, quote, rparen, tag, tags, token, tokensToPush, val, value;
        ({quote, indent, double} = options);
        attachQuote = function(token) {
          token[1] = preserveTagged(token)(new String(token[1]));
          return token[1].quote = quote;
        };
        attachDouble = function(token) {
          if (!double) {
            return;
          }
          token[1] = preserveTagged(token)(new String(token[1]));
          return token[1].double = double;
        };
        markInitialChunk = function(token) {
          token[1] = preserveTagged(token)(new String(token[1]));
          return token[1].initialChunk = true;
        };
        markFinalChunk = function(token) {
          token[1] = preserveTagged(token)(new String(token[1]));
          return token[1].finalChunk = true;
        };
        markIndent = function(token) {
          if (!indent) {
            return;
          }
          token[1] = preserveTagged(token)(new String(token[1]));
          return token[1].indent = indent;
        };
        tags = ['quote', 'initialChunk', 'finalChunk', 'indent', 'double'];
        preserveTagged = function(token) {
