...
(function() {
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    identifierToken() {
      var alias, badCSXSpread, colon, colonOffset, colonToken, expected, id, idLength, inCSXTag, input, match, poppedToken, prev, prevprev, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, regex, tag, tagToken;
      inCSXTag = this.atCSXTag();
      if (inCSXTag && this.prev()[1] !== ':' && ((ref = this.chunk[0]) === '{' || ref === '"' || ref === "'")) {
        if ((ref1 = this.chunk[0]) === '"' || ref1 === "'") {
          this.error("Unexpected token");
        }
        if (!(CSX_SPREAD_LEFT.exec(this.chunk) || CSX_SPREAD_RIGHT.exec(this.chunk))) {
          if (badCSXSpread = CSX_FIND_ATTRIBUTE_ERROR.exec(this.chunk)) {
            expected = badCSXSpread[1] === '...' || badCSXSpread[3] === '...' ? "}" : "...";
            this.error(`Unexpected token, expected ${expected}`, {
              offset: badCSXSpread[0].length - 1
            });
          }
        }
      }
      ...
      if (tag === 'IDENTIFIER' && (indexOf.call(JS_KEYWORDS, id) >= 0 || indexOf.call(COFFEE_KEYWORDS, id) >= 0) && !(this.exportSpecifierList && indexOf.call(COFFEE_KEYWORDS, id) >= 0)) {
        ...
        if (prev.spaced && (ref8 = prev[0], indexOf.call(CALLABLE, ref8) >= 0) && /^[gs]et$/.test(prev[1])) {
          ...
          if (((ref9 = prev[0]) === '@' || ref9 === 'THIS') && prevprev && prevprev.spaced && /^[gs]et$/.test(prevprev[1]) && this.tokens[this.tokens.length - 3][0] !== '.') {
  ...
  CSX_SPREAD_RIGHT = /^{\s*((?:(?!\s)[$\w\x7f-\uffff])+)\.{3}\s*}/;

  CSX_SPREAD_LEFT = /^{\s*\.{3}((?:(?!\s)[$\w\x7f-\uffff])+)\s*}/;

  CSX_FIND_ATTRIBUTE_ERROR = /^{\s*(?!\d)(\.{3})?((?:(?!\s)[$\w\x7f-\uffff])+)(\.{3})?([:=;,\s}])/;
