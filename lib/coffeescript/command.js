...
(function() {
  ...
  var BANNER, CoffeeScript, EventEmitter, SWITCHES, buildCSOptionParser, compileJoin, compileOptions, compilePath, compileScript, compileStdio, exec, findDirectoryIndex, forkNode, fs, helpers, hidden, joinTimeout, makePrelude, mkdirp, notSources, optionParser, optparse, opts, outputPath, parseOptions, path, printLine, printTokens, printWarn, readline, removeSource, removeSourceDir, silentUnlink, sourceCode, sources, spawn, streamInput, timeLog, usage, useWinPathSep, version, wait, watch, watchDir, watchedDirs, writeJs,
    indexOf = [].indexOf;
...
'--' will be removed from the argument list.`);
    ...
    if (opts.eval || opts['stream-input']) {
...
`);
  ...
  compileScript = function(file, input, base = null) {
    ...
    try {
      ...
      } else if (opts.run || opts['stream-input']) {
        CoffeeScript.register();
        if (opts.prelude) {
          CoffeeScript.eval(opts.prelude, task.options);
        }
        if (opts['stream-input']) {
          return streamInput(task.input, task.options);
        } else {
  ...
  // the current line.
  streamInput = function(input, options) {
    var rl;
    options['stream-input'] = true;
    if (!input) {
    ...
    return rl.on('line', function(line) {
      ...
      return CoffeeScript.eval(input, options);
    });
  };
