...
(function() {
  ...
  var BALANCED_PAIRS, CALL_CLOSERS, DISCARDED, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_CALL, IMPLICIT_END, IMPLICIT_FUNC, IMPLICIT_UNSPACED_CALL, INVERSES, LINEBREAKS, Rewriter, SINGLE_CLOSERS, SINGLE_LINERS, generate, k, left, len, moveComments, right, throwSyntaxError,
    indexOf = [].indexOf;

  ({throwSyntaxError} = require('./helpers'));

  moveComments = function(fromToken, toToken) {
    var comment, k, len, ref;
      ...
      rescueStowawayComments() {
        ...
        return this.scanTokens(function(token, i, tokens) {
          ...
          if (ref = token[0], indexOf.call(DISCARDED, ref) >= 0) {
            ...
            while (j !== -1) {
              if (token.comments[j].newLine && ((token.comments[j].unshift === true && i !== tokens.length - 1 && (ref2 = tokens[i + 1][0], indexOf.call(LINEBREAKS, ref2) < 0)) || (token.comments[j].unshift !== true && i !== 0 && (ref3 = tokens[i - 1][0], indexOf.call(LINEBREAKS, ref3) < 0)))) {
  ...
  DISCARDED = LINEBREAKS.concat(['CALL_END', '->', '=>', '{', '}']);
