...
(function() {
  ...
  var BALANCED_PAIRS, CALL_CLOSERS, CONTROL_IN_IMPLICIT, DISCARDED, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_CALL, IMPLICIT_END, IMPLICIT_FUNC, IMPLICIT_UNSPACED_CALL, INVERSES, LINEBREAKS, Rewriter, SINGLE_CLOSERS, SINGLE_LINERS, generate, k, l, left, len, len1, moveComments, pair, right, throwSyntaxError,
    indexOf = [].indexOf;
...
// Helper functions
          ...
          startImplicitCall = function(idx) {
            ...
            return tokens.splice(idx, 0, generate('CALL_START', '(', null, prevToken));
          };
          ...
          implicitObjectContinues = (j) => {
            ...
            return this.looksObjectish(nextTerminatorIdx + 1);// Don’t end an implicit call/object on next indent if any of these are in an argument/value.
          };
...
// All following tokens are doomed!
        ...
        return this.scanTokens(function(token, i, tokens) {
          ...
          if (!(token.comments && !token.generated)) {
            return 1;
          }
...
// comment is output as starting a line.
            ...
            while (j !== -1) {
              if (token.comments[j].newLine && !token.comments[j].unshift && (ref1 = (ref2 = tokens[i + 1]) != null ? ref2[0] : void 0, indexOf.call(LINEBREAKS, ref1) >= 0)) {
...
// argument doesn’t contain a `new` call for these tokens.
  DISCARDED = ['->', '=>', '.', '..', '...', ',', '=', '++', '--', '?', 'AS', 'AWAIT', 'DEFAULT', 'ELSE', 'EXTENDS', 'EXPORT', 'FORIN', 'FOROF', 'FORFROM', 'IMPORT', 'INDEX_SOAK', 'LEADING_WHEN', 'RETURN', 'SUPER', 'THROW', 'YIELD'].concat(IMPLICIT_UNSPACED_CALL.concat(IMPLICIT_END.concat(CALL_CLOSERS.concat(CONTROL_IN_IMPLICIT))));

  for (l = 0, len1 = BALANCED_PAIRS.length; l < len1; l++) {
    pair = BALANCED_PAIRS[l];
    DISCARDED = DISCARDED.concat(pair);
  }
