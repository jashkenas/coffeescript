  ...
  exports.Rewriter = Rewriter = (function() {
    class Rewriter {
      ...
      addParensToConditions() {
        var conditionTags, endTags, findGlyph, glyphTags, wrapInParens;
        conditionTags = ['IF', 'UNLESS', 'WHILE', 'UNTIL', 'SWITCH'];
        endTags = ['ELSE', 'THEN', ...LINEBREAKS];
        glyphTags = ['->', '=>'];
        findGlyph = (idx) => {
          return this.detectEnd(idx, function(token) {
            var ref;
            return ref = token[0], indexOf.call([...glyphTags, ...endTags], ref) >= 0;
          }, function(token, i) {
            var ref, ref1;
            if ((ref = token[0], indexOf.call(glyphTags, ref) >= 0) && (ref1 = this.tag(i + 1), indexOf.call(LINEBREAKS, ref1) < 0)) {
              return i;
            }
          }, {
            returnOnNegativeLevel: true
        ...
        wrapInParens = (startIndex, idx) => {
          return this.detectEnd(idx, function(token) {
            var ref;
            return ref = token[0], indexOf.call(endTags, ref) >= 0;
          }, function(token, i) {
            var endIndex;
            endIndex = i + 1;
            this.tokens.splice(startIndex, 0, generate('(', '(', this.tokens[startIndex]));
            return this.tokens.splice(endIndex, 0, generate(')', ')', this.tokens[endIndex]));
          }, {
            returnOnNegativeLevel: true
          });
        };
        return this.scanTokens(function(token, i, tokens) {
          var funcGlyphIndex, nextTag, startIndex, tag;
          startIndex = funcGlyphIndex = null;
          [tag] = token;
          nextTag = this.tag(i + 1);
          if (!(indexOf.call(conditionTags, tag) >= 0 && nextTag !== '(')) {
            return 1;
          }
          // Open parens index.
          startIndex = i + 1;
