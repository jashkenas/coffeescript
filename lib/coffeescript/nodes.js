...
(function() {
  ...
  exports.Range = Range = (function() {
    class Range extends Base {
      ...
      compileArray(o) {
        var args, body, cond, hasArgs, i, idt, j, known, post, pre, range, ref1, ref2, result, results1, vars;
        known = (this.fromNum != null) && (this.toNum != null);
  ...
  exports.Obj = Obj = (function() {
    class Obj extends Base {
      ...
      compileNode(o) {
        var answer, closeBracket, hasSpread, i, idt, indent, isCompact, j, join, k, key, l, lastNoncom, len1, len2, len3, len4, len5, node, openBracket, p, prop, props, q, ref1, s, spreadAnswer, spreads, value;
        props = this.properties;
        if (this.generated) {
          for (j = 0, len1 = props.length; j < len1; j++) {
            node = props[j];
        ...
        spreadAnswer = [];
        spreads = [];
        openBracket = this.makeCode("{");
        closeBracket = this.makeCode("}");
        hasSpread = false;
        answer = [];
        ...
        for (i = l = 0, len3 = props.length; l < len3; i = ++l) {
          ...
          if (key instanceof Value && key.hasProperties() && !(key instanceof Splat)) {
          ...
          if (key === prop && !(prop instanceof Splat)) {
            if (prop.shouldCache()) {
          ...
          if (key === prop && prop instanceof Splat) {
            if (spreadAnswer.length) {
              spreadAnswer.push(this.makeCode(", "));
            }
            if (spreads.length) {
              spreadAnswer.push(openBracket);
              for (p = 0, len4 = spreads.length; p < len4; p++) {
                s = spreads[p];
                spreadAnswer.push(...s.compileToFragments(o, LEVEL_TOP), this.makeCode(", "));
              }
              spreadAnswer.pop();
              spreadAnswer.push(closeBracket, this.makeCode(", "));
            }
        ...
        if (hasSpread) {
          if (spreads.length) {
            spreadAnswer.push(this.makeCode(", "), openBracket);
            for (q = 0, len5 = spreads.length; q < len5; q++) {
              s = spreads[q];
              spreadAnswer.push(...s.compileToFragments(o, LEVEL_TOP), this.makeCode(", "));
            }
            spreadAnswer.pop();
          ...
          answer = [this.makeCode("Object.assign({}, "), ...spreadAnswer, this.makeCode(")")];
        } else {
          answer.push(this.makeCode(`${(isCompact ? '' : `\n${this.tab}`)}}`));
        }
        if (this.front) {
      ...
      proxyBoundMethods(o) {
        ...
        this.ctor.thisAssignments = (function() {
          ...
          for (j = ref1.length - 1; j >= 0; j += -1) {
            ...
            results1.push(new Literal(`${name} = ${utility('bind', o)}(${name}, this)`));
          }
          return results1;
        }).call(this);
        return null;
  ...
  exports.Assign = Assign = (function() {
    class Assign extends Base {
      constructor(variable1, value1, context1, options = {}) {
        ...
        ({param: this.param, paramWithSplat: this.paramWithSplat, subpattern: this.subpattern, operatorToken: this.operatorToken, moduleDeclaration: this.moduleDeclaration} = options);
      }
      ...
      objectHasSplat(o, obj) {
        return obj.contains(function(n) {
          return n instanceof Splat;
        });
      }

      valueHasSplat(o) {
        return this.objectHasSplat(o, this.value);
      }

      variableHasSplat(o) {
        return this.objectHasSplat(o, this.variable);
      }

      compileNode(o) {
        var answer, answers, answersOnTop, compiledName, isValue, j, name, properties, prototype, ref, ref1, ref2, ref3, ref4, ref5, ref6, restElements, val, varBase, vvarText;
        answers = [];
        answersOnTop = [];
        ...
        if (isValue) {
          ...
          if (this.variable.isArray() || this.variable.isObject()) {
            ...
            if (this.variable.isObject()) {
              if (this.variableHasSplat(o)) {
                val = this.value.compileToFragments(o, LEVEL_LIST);
                vvarText = fragmentsToText(val);
                if (!(this.value.unwrap() instanceof IdentifierLiteral) || this.variable.assigns(vvarText)) {
                  answersOnTop.push([this.makeCode(`${(ref = o.scope.freeVariable('ref'))} = `), ...val]);
                  this.value = new IdentifierLiteral(ref);
                }
              }
              restElements = this.compileObjectDestruct(o);
        ...
        if (o.level > LEVEL_LIST || (isValue && this.variable.base instanceof Obj && (!this.param || this.paramWithSplat))) {
          answers.unshift(this.wrapInParentheses(answer));
        } else {
          answers.unshift(answer);
        }
        return this.joinFragmentArrays([...answersOnTop, ...answers], ', ');
      }

      compileObjectDestruct(o) {
        var answers, extractKeys, getPropValue, j, len1, objects, prop, restElement, restList, traverseRest, val, varProp, vvarPropText, vvarText;
        getPropValue = function(obj) {
          return fragmentsToText((obj instanceof Assign ? obj.variable.unwrapAll() : obj.unwrap()).compileToFragments(o));
        };
        traverseRest = function(objects, props = []) {
          var excludeProps, j, key, len1, obj, prop, restElement, results;
          results = [];
          restElement = false;
          for (key = j = 0, len1 = objects.length; j < len1; key = ++j) {
            obj = objects[key];
            if (obj instanceof Assign && obj.context === "object" && obj.value.base instanceof Obj) {
            ...
            if (obj instanceof Splat) {
              if (restElement) {
                obj.error("multiple rest elements are disallowed in object destructuring");
              }
              restElement = {
                key,
                name: obj.unwrap(),
          ...
          if (restElement) {
            ...
            excludeProps = (function() {
              var k, len2, results1;
              results1 = [];
              for (k = 0, len2 = objects.length; k < len2; k++) {
                obj = objects[k];
                results1.push(getPropValue(obj));
              }
              return results1;
            })();
            excludeProps = (function() {
              var k, len2, results1;
              results1 = [];
              for (k = 0, len2 = excludeProps.length; k < len2; k++) {
                prop = excludeProps[k];
                results1.push(prop[0] === "`" ? prop : `'${prop.replace(/\'/g, "")}'`);
              }
              return results1;
            })();
        ...
        val = this.value.compileToFragments(o, LEVEL_LIST);
        vvarText = fragmentsToText(val);
        for (j = 0, len1 = restList.length; j < len1; j++) {
          restElement = restList[j];
          varProp = ((function() {
            var k, len2, ref1, results1;
            ref1 = restElement.props;
            results1 = [];
            for (k = 0, len2 = ref1.length; k < len2; k++) {
              prop = ref1[k];
              results1.push(prop[0] === "`" ? `[${prop}]` : `['${prop.replace(/\'/g, "")}']`);
    ...
    class Code extends Base {
      ...
      compileNode(o) {
            ...
            if (!haveSplatParam) {
              ...
              if (param.name instanceof Arr || param.name instanceof Obj) {
                ...
                if (param.name instanceof Obj) {
                  traverseRest = function(objects) {
                    var k, key, len2, obj, propParams, restElement, results;
                    results = {
                      splats: [],
                      allProps: []
                    };
                    restElement = false;
                    propParams = [];
                    for (key = k = 0, len2 = objects.length; k < len2; key = ++k) {
                      obj = objects[key];
                      if (obj instanceof Assign && obj.context === "object" && obj.value.base instanceof Obj) {
                      ...
                      if (obj instanceof Splat) {
                        ...
                        if (!(obj instanceof Assign)) {
                          ...
                          if (obj.value.base instanceof IdentifierLiteral) {
                            propParams.push(obj.value.unwrap().value);
                          }
                          if (obj.value instanceof Assign && obj.value.variable.base instanceof IdentifierLiteral) {
                            propParams.push(obj.value.variable.unwrap().value);
                          }
                        }
                      }
                    }
                    ...
                    if (restElement) {
                      results.splats.push(restElement);
                    }
                    return results;
                  };
                  objParams = traverseRest(param.name.objects);
                  if (objParams.splats.length) {
                    ref5 = objParams.allProps;
                    for (k = 0, len2 = ref5.length; k < len2; k++) {
                      val = ref5[k];
                      o.scope.add(val, 'var', true);
                    }
                    ref = param.asReference(o);
                    exprs.push(new Assign(new Value(param.name), ref, null, {
                      param: true,
                ...
                if (param.name instanceof Arr && param.shouldCache()) {
                  arrParams = [];
                  ref6 = param.name.objects;
                  for (l = 0, len3 = ref6.length; l < len3; l++) {
                    prop = ref6[l];
                    if (prop instanceof Assign) {
                      arrParams.push(prop.value.base instanceof IdentifierLiteral ? prop.value.base.value : prop.variable.base.value);
                    } else {
                      arrParams.push(prop instanceof Splat ? prop.name.unwrap().value : prop.unwrap().value);
                    }
                  }
                  for (p = 0, len4 = arrParams.length; p < len4; p++) {
                    val = arrParams[p];
                    o.scope.add(val, 'var', true);
                  }
                }
        ...
        if (paramsAfterSplat.length !== 0) {
          exprs.unshift(new Assign(new Value(new Arr([
            new Splat(new IdentifierLiteral(splatParamName)), ...(function() {
              var len5, q, results1;
              results1 = [];
              for (q = 0, len5 = paramsAfterSplat.length; q < len5; q++) {
                param = paramsAfterSplat[q];
        ...
        answer = this.joinFragmentArrays((function() {
          var len6, r, results1;
          results1 = [];
          for (r = 0, len6 = modifiers.length; r < len6; r++) {
            m = modifiers[r];
  ...
  exports.Param = Param = (function() {
    class Param extends Base {
      ...
      asReference(o) {
        ...
        } else if (node.shouldCache() || node.lhs) {
