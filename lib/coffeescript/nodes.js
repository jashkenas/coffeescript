...
(function() {
  ...
  ({compact, flatten, extend, merge, del, starts, ends, some, addDataToNode, attachCommentsToNode, locationDataToString, throwSyntaxError, dump} = require('./helpers'));
  ...
  exports.StringLiteral = StringLiteral = class StringLiteral extends Literal {
    constructor(value, {quote, initialChunk, finalChunk} = {}) {
      ...
      dump({value, initialChunk: this.initialChunk, finalChunk: this.finalChunk});
    }

    formatValue() {
      return this.value.replace(SIMPLE_STRING_OMIT, (match, offset) => {
        if ((this.initialChunk && offset === 0) || (this.finalChunk && offset + match.length === value.length)) {
          return '';
        } else {
          return ' ';
        }
