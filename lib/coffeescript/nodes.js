...
(function() {
  ...
  exports.Base = Base = (function() {
    class Base {
      ...
      eachChild(func) {
        var attr, child, j, l, len1, len2, ref3, ref4;
        if (!this.children) {
          return this;
        }
        ref3 = this.children;
      ...
      replaceInContext(match, replacement) {
        var attr, child, children, i, j, l, len1, len2, ref3, ref4;
        if (!this.children) {
          return false;
        }
        ref3 = this.children;
  ...
  exports.Obj = Obj = (function() {
    class Obj extends Base {
      ...
      compileNode(o) {
        var answer, i, idt, indent, j, join, key, l, lastNoncom, len1, len2, node, prop, props, ref3, value;
        props = this.properties;
        if (this.generated) {
          for (j = 0, len1 = props.length; j < len1; j++) {
            node = props[j];
        ...
        for (i = l = 0, len2 = props.length; l < len2; i = ++l) {
          ...
          if (!(prop instanceof Comment) && !(prop instanceof Assign) && !(prop instanceof Splat)) {
            if (prop.shouldCache()) {
  ...
  exports.Class = Class = (function() {
    class Class extends Base {
      ...
      walkBody() {
        var assign, end, executableBody, expression, expressions, exprs, i, initializer, initializerExpression, j, l, len1, len2, method, properties, pushSlice, ref3, start;
        this.ctor = null;
        this.boundMethods = [];
        executableBody = null;
        initializer = [];
        ...
        if (initializer.length !== expressions.length) {
          this.body.expressions = (function() {
            var len3, p, results;
            results = [];
            for (p = 0, len3 = initializer.length; p < len3; p++) {
              expression = initializer[p];
  ...
  exports.Assign = Assign = (function() {
    class Assign extends Base {
      ...
      compilePatternMatch(o) {
        var acc, assigns, code, defaultValue, expandedIdx, extractObjectWithoutKeys, fragments, i, idx, isObject, ivar, j, k, lastSplat, len1, message, name, nonSplatKeys, obj, objects, olen, ref, ref3, ref4, ref5, ref6, rest, splatErrors, splatKey, splatList, top, val, value, vvar, vvarText;
        top = o.level === LEVEL_TOP;
        value = this.value;
        ...
        if (isObject) {
          ...
          lastSplat = splatList[splatList.length - 1];
          if (splatList.length > 1) {
            splatErrors.push("multiple rest elements are disallowed in object destructuring");
          }
          if (lastSplat < olen - 1) {
            splatErrors.push("rest element has to be the last element when destructuring");
          }
        ...
        for (i = j = 0, len1 = objects.length; j < len1; i = ++j) {
          obj = objects[i];
          idx = i;
          if (!expandedIdx && obj instanceof Splat && !isObject) {
            ...
            if ((obj instanceof Splat || obj instanceof Expansion) && !isObject) {
              obj.error("multiple splats/expansions are disallowed in an assignment");
            }
            defaultValue = null;
            if (obj instanceof Assign && obj.context === 'object') {
              if (obj !== splatKey) {
                nonSplatKeys.push(obj.variable.unwrap().value);
              }
              ...
              if (obj !== splatKey) {
                nonSplatKeys.push(obj.unwrap().value);
              }
        ...
        if (splatKey) {
          nonSplatKeys = (function() {
            var l, len2, results;
            results = [];
            for (l = 0, len2 = nonSplatKeys.length; l < len2; l++) {
              k = nonSplatKeys[l];
              results.push(k !== void 0 ? `'${k.replace(/\'/g, '')}'` : k);
            }
            return results;
          })();
          extractObjectWithoutKeys = new Literal(`${utility('extractObjectWithoutKeys', o)}(${vvarText}, [${nonSplatKeys}])`);
          assigns.push(new Assign(splatKey.unwrap(), extractObjectWithoutKeys, null, {
            param: this.param,
            subpattern: true
          }).compileToFragments(o, LEVEL_LIST));
        }
  ...
  exports.Code = Code = (function() {
    class Code extends Base {
      ...
      compileNode(o) {
        var answer, body, condition, exprs, haveBodyParam, haveSplatParam, i, ifTrue, j, l, len1, len2, m, methodScope, modifiers, name, param, paramNames, params, paramsAfterSplat, ref, ref3, ref4, ref5, ref6, ref7, ref8, signature, splatParamName, thisAssignments, wasEmpty;
        if (this.ctor) {
          if (this.isAsync) {
        ...
        if (paramsAfterSplat.length !== 0) {
          exprs.unshift(new Assign(new Value(new Arr([
            new Splat(new IdentifierLiteral(splatParamName)), ...(function() {
              var l, len2, results;
              results = [];
              for (l = 0, len2 = paramsAfterSplat.length; l < len2; l++) {
                param = paramsAfterSplat[l];
        ...
        for (i = l = 0, len2 = params.length; l < len2; i = ++l) {
          param = params[i];
          if (i) {
        ...
        answer = this.joinFragmentArrays((function() {
          var len3, p, results;
          results = [];
          for (p = 0, len3 = modifiers.length; p < len3; p++) {
            m = modifiers[p];
  ...
  exports.Switch = Switch = (function() {
    class Switch extends Base {
      ...
      compileNode(o) {
        var block, body, cond, conditions, expr, fragments, i, idt1, idt2, j, l, len1, len2, ref3, ref4, ref5;
        idt1 = o.indent + TAB;
  ...
  UTILITIES = {
    ...
    extractObjectWithoutKeys: function(o) {
      return `function (obj, keys) { var target = {}; for (var i in obj) { if (keys.indexOf(i) >= 0) continue; if (!${utility('hasProp', o)}.call(obj, i)) continue; target[i] = obj[i]; } return target; }`;
    },
    modulo: function() {
