...
// multiline need to be indented.
  ...
  makeDelimitedLiteral = function(body, {
    ...
    regex = RegExp(`(\\\\\\\\)|(\\\\0(?=\\d))${convertTrailingNullEscapes ? "|(\\\\0)$" : '' // Escaped backslash. // Trailing null character that could be mistaken as octal escape.
    ...
    // Other escapes.
}${escapeDelimiter ? `|\\\\?(${delimiterOption})` : ''}${escapeTemplateLiteralCurlies ? "|\\\\?(\\$\\{)" : '' // `${` inside template literals must be escaped.
}|\\\\?(?:${escapeNewlines ? '(\n)|' : ''}(\\r)|(\\u2028)|(\\u2029))|(\\\\.)`, "g");
