  ...
  exports.Assign = Assign = (function() {
    class Assign extends Base {
      ...
      compileNode(o) {
        ...
        if (isValue) {
          ...
          if (this.variable.isArray() || this.variable.isObject()) {
            ...
            if (this.variable.isObject() && this.variable.contains(function(n) {
      ...
      compileObjectDestruct(o) {
        var compiledName, extractKeys, fragments, getPropValue, j, len1, objVar, properties, ref, restElement, restList, restSource, setScopeVar, traverseRest, val, varProp, vvarPropText, vvarText;
        ...
        getPropValue = function(prop, quote = false) {
          var wrapInQutes;
          wrapInQutes = function(prop) {
            var compiledProp;
            compiledProp = prop.compile(o);
            if (quote) {
              compiledProp = `'${compiledProp}'`;
            }
            return (new Literal(compiledProp)).compile(o);
          };
          if (prop instanceof Assign) {
            if (prop.variable.base instanceof StringWithInterpolations || prop.variable.base instanceof StringLiteral) {
              return prop.variable.compile(o);
            }
        ...
        traverseRest = function(properties, defaultValue, path = []) {
          var base1, j, key, len1, nestedProperties, prop, restElement, restKey, results, val;
          results = [];
          restElement = false;
          for (key = j = 0, len1 = properties.length; j < len1; key = ++j) {
            prop = properties[key];
            setScopeVar(prop.unwrap());
            if (prop instanceof Assign) {
              nestedProperties = (typeof (base1 = prop.value).isObject === "function" ? base1.isObject() : void 0) ? prop.value.base.properties : prop.value instanceof Assign && prop.value.variable.isObject() ? ([val, defaultValue] = prop.value.value.cache(), prop.value.value = val, prop.value.variable.base.properties) : void 0;
              if (nestedProperties) {
          ...
          if (restElement) {
            ...
            restElement["excludeProps"] = new Literal(`[${(function() {
              ...
              for (k = 0, len2 = properties.length; k < len2; k++) {
                prop = properties[k];
                results1.push(getPropValue(prop, true));
              }
              return results1;
            })()}]`);
        ...
        if ((!(this.value.unwrap() instanceof IdentifierLiteral)) || this.variable.assigns(vvarText)) {
          ...
          fragments.push([this.makeCode(ref + ' = '), ...val]);
        ...
        for (j = 0, len1 = restList.length; j < len1; j++) {
          ...
          varProp = restElement.path.length ? `.${restElement.path.join('.')}` : "";
          restSource = vvarPropText = new Literal(`${vvarText}${varProp}`);
          if (restElement.defaultValue) {
            restSource = new Op('?', vvarPropText, restElement.defaultValue);
          }
