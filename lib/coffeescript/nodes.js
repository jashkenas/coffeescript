...
(function() {
  ...
  exports.SubStructAssign = SubStructAssign = class SubStructAssign extends Base {
    constructor(variable1, value1, lhs1 = false) {
      super();
      this.variable = variable1;
      this.value = value1;
      this.lhs = lhs1;
      [this.objVar, this.objVal] = this.variable;
    }
    ...
    objProperties(props) {
      ...
      for (j = 0, len1 = props.length; j < len1; j++) {
        ...
        if (prop instanceof Assign) {
          ...
          ret.push(prop.name);
        } else if (!(prop.base instanceof ExProperty)) {
          ret.push(prop.base);
        }
      }
      return ret;
    ...
    finalProps(props = []) {
      var getProps, j, len1, prop, ret;
      ret = [];
      getProps = (props) => {
        return this.finalProps(this.objProperties(props));
      };
      for (j = 0, len1 = props.length; j < len1; j++) {
        prop = props[j];
    ...
    compileNode(o) {
      var answer, body, fProps;
      fProps = this.finalProps(this.objProperties(this.objVal.properties));
      body = [new Assign(new Value(this.objVal), new Value(this.objVar))];
      if (this.lhs) {
        body.push(new Value(new Obj([new Splat(this.value), ...fProps])));
      } else {
        body.push(new Obj(fProps));
      }
      answer = new Code([], Block.wrap(body), new FuncGlyph('->'));
      answer = new Call(new Parens(answer), [], false);
      return answer.compileToFragments(o, LEVEL_TOP);
    }
  ...
  exports.ExProperty = ExProperty = class ExProperty extends PropertyName {
    compileNode(o) {
      return (new Value(this.value)).compileToFragments(o, LEVEL_LIST);
    }
...
// multiline need to be indented.
  ...
  exports.Obj = Obj = (function() {
    class Obj extends Base {
      ...
      hasExProperty() {
        ...
        for (j = 0, len1 = ref1.length; j < len1; j++) {
          prop = ref1[j];
          if (prop.base instanceof ExProperty) {
            return true;
          }
        }
        return false;
      ...
      excludeProperties(props) {
        ...
        for (j = 0, len1 = props.length; j < len1; j++) {
          p = props[j];
          if (!(p.base instanceof ExProperty)) {
            results.push(p);
          }
        }
        return results;
      ...
      compileNode(o) {
        ...
        this.objects = this.properties = this.excludeProperties(this.properties);
        props = this.properties;
        if (this.generated) {
          for (j = 0, len1 = props.length; j < len1; j++) {
            node = props[j];
  ...
  exports.Assign = Assign = (function() {
    class Assign extends Base {
      ...
      compileNode(o) {
        var answer, compiledName, hasSplat, isValue, name, objDestructAnswer, objVal, objVar, properties, prototype, ref1, ref2, ref3, ref4, ref5, subStruct, subStructAssign, val, varBase;
        isValue = this.variable instanceof Value;
        // Convert LHS sub-structuring assignment.
        if (this.variable.base instanceof SubStructAssign) {
          ({objVar, objVal} = this.variable.base);
          if (!o.scope.check(objVar.value)) {
            this.error(`the variable '${objVar.value}' has not been declared before`);
          }
          subStruct = new SubStructAssign([this.value, objVal], objVar, true);
