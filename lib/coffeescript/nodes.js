...
(function() {
  ...
  exports.Obj = Obj = (function() {
    class Obj extends Base {
      ...
      compileNode(o) {
        var answer, i, idt, indent, isCompact, j, join, k, key, l, lastNoncom, len1, len2, len3, len4, node, prop, propHasSplat, propVal, props, q, ref1, unwrappedVal, value;
        props = this.properties;
        if (this.generated) {
          for (j = 0, len1 = props.length; j < len1; j++) {
            node = props[j];
        ...
        for (i = q = 0, len4 = props.length; q < len4; i = ++q) {
          ...
          if (key === prop) {
            if (!propHasSplat) {
              ...
              prop = this.csx ? new Literal(`{${prop.compile(o)}}`) : prop;
            }
          }
          if (this.csx && prop instanceof Assign) {
            if (!(prop.variable.unwrap() instanceof PropertyName)) {
              prop.variable.error("Unexpected token");
            }
            propVal = prop.value.unwrap();
            if (!(((propVal instanceof Parens || propVal instanceof StringWithInterpolations) && propVal.body instanceof Block) || propVal instanceof StringLiteral || propVal instanceof Obj)) {
