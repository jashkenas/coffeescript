...
(function() {
  ...
  exports.Assign = Assign = (function() {
    class Assign extends Base {
      ...
      compileNode(o) {
        var answer, compiledName, isValue, j, key, lastSplat, name, obj, objects, properties, prototype, ref1, ref2, ref3, ref4, ref5, ref6, restElement, restErrors, restList, val, varBase;
        restElement = false;
        restErrors = [];
          ...
          if (this.variable.isArray() || this.variable.isObject()) {
            ...
            if (this.variable.isObject()) {
              ...
              if (restList.length > 0) {
                lastSplat = restList[restList.length - 1];
                if (restList.length > 1) {
                  restErrors.push("multiple rest elements are disallowed in object destructuring");
                }
                ...
                this.variable.base = new Obj(objects, false);
              }
            }
          }
        ...
        var vvarText = fragmentsToText(val);
        ...
        if (o.level > LEVEL_LIST || (isValue && this.variable.base instanceof Obj)) {
          // return this.wrapInBraces(answer);
          answers.push(this.wrapInBraces(answer));  
        } else {
          // return answer;
          answers.push(answer)
        }
        ...
        if (restElement) {
          var excludeProps = objects
                              .map((obj) => (obj instanceof Assign ? obj.variable.unwrap().value : obj.unwrap().value))
                              .map((prop) => `'${prop.replace(/\'/g, "")}'`)
          var extractKeys = new Literal(`Object.keys(${vvarText}).reduce(function(a,c) { if (![${excludeProps}].includes(c)) a[c] = ${vvarText}[c]; return a;  }, {})`)  
          var restVar = new Assign(restElement.unwrap(), extractKeys, null).compileToFragments(o, LEVEL_TOP)  
          answers.push(restVar)
        }
        var fragments = this.joinFragmentArrays(answers, ', ');
        return fragments
      }
