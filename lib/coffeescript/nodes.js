...
(function() {
  ...
  ({compact, flatten, extend, merge, del, starts, ends, some, addDataToNode, attachCommentsToNode, locationDataToString, throwSyntaxError, makeDelimitedLiteral, replaceUnicodeCodePointEscapes} = require('./helpers'));
  ...
  exports.RegexLiteral = RegexLiteral = class RegexLiteral extends Literal {
    constructor(value, {delimiter: delimiter = '/'} = {}) {
      super('');
    ...
    formatValue(val) {
      var endDelimiterIndex, heregex;
      heregex = this.delimiter === '///';
      endDelimiterIndex = val.lastIndexOf('/');
      this.flags = val.slice(endDelimiterIndex + 1);
      return this.value = (() => {
        val = this.originalValue = val.slice(1, endDelimiterIndex);
        if (heregex) {
          val = val.replace(HEREGEX_OMIT, '$1$2');
        }
