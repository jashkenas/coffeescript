...
// multiline need to be indented.
  ...
  makeDelimitedLiteral = function(body, {
    ...
    regex = RegExp(`(\\\\\\\\)|(\\\\0(?=[1-7]))${escapeDelimiter ? `|\\\\?(${delimiterOption // Escaped backslash.
    ...
    // Other escapes.
})` : ''}${escapeTemplateLiteralCurlies ? "|\\\\?(\\$\\{)" : '' // `${` inside template literals must be escaped.
}|\\\\?(?:${escapeNewlines ? '(\n)|' : ''}(\\r)|(\\u2028)|(\\u2029))|(\\\\.)`, "g");
    body = body.replace(regex, function(match, backslash, nul, ...args) {
      var cr, delimiter, lf, ls, other, ps, templateLiteralCurly;
