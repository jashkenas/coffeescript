...
// the syntax tree into a string of JavaScript code, call `compile()` on the root.
  ...
  ({compact, flatten, extend, merge, del, starts, ends, some, addDataToNode, locationDataToString, throwSyntaxError} = require('./helpers'));
  ...
  exports.Base = Base = (function() {
    class Base {
      ...
      compileToFragments(o, lvl) {
        ...
        if (node.comments) {
          this.attachComments(o, node, fragments);
        }
        return fragments;
      }
...
// object with their parent closure, to preserve the expected lexical scope.
      ...
      attachComments(o, node, fragments) {
        var code, codes, comment, commentFragment, fragment, fragmentIndex, hasLeadingMarks, j, k, largestIndent, leadingWhitespace, len1, len2, line, multiline, ref1, ref2, results;
        ref1 = node.comments;
        results = [];
        for (j = 0, len1 = ref1.length; j < len1; j++) {
          comment = ref1[j];
          if (!(indexOf.call(this.attachedComments, comment) < 0)) {
            continue;
          }
...
// Unindent multiline comments. They will be reindented later.
            if (multiline) {
              ...
              code = code.replace(RegExp(`^(${leadingWhitespace})`, "gm"), '');
            }
            code = `/*${code}${(hasLeadingMarks ? ' ' : '')}*/`;
            if (comment.newLine) {
              code = `${code}\n`;
            }
            commentFragment = this.makeCode(code);
            commentFragment.type = 'HereComment';
            commentFragment.multiline = multiline;
            commentFragment.suppressTrailingSemicolon = true;
            if (comment.unshift) {
              results.push((function() {
                var l, len3, results1;
                results1 = [];
                for (fragmentIndex = l = 0, len3 = fragments.length; l < len3; fragmentIndex = ++l) {
                  fragment = fragments[fragmentIndex];
                  if (!(fragment.type !== 'HereComment')) {
                    continue;
                  }
              ...
              results.push(fragments.push(commentFragment));
            }
          } else {
...
// might be multiline.
            ...
            code = code.replace(/\n+$/, '');
            code = `//${code}`;
            codes = multiline ? code.split('\n') : [code];
            results.push((function() {
              var base1, base2, l, len3, results1;
              results1 = [];
              for (l = 0, len3 = codes.length; l < len3; l++) {
                code = codes[l];
                if (!(code !== '')) {
                  continue;
                }
                ...
                commentFragment.trail = !comment.newLine && !comment.unshift;
                if (comment.unshift) {
                  if ((base1 = fragments[0]).precedingComments == null) {
                    base1.precedingComments = [];
                  }
                  results1.push(fragments[0].precedingComments.push(commentFragment));
                } else {
                  if ((base2 = fragments[fragments.length - 1]).trailingComments == null) {
                    base2.trailingComments = [];
                  }
                  results1.push(fragments[fragments.length - 1].trailingComments.push(commentFragment));
                }
              }
              return results1;
            }).call(this));
...
// Track which comments have been output.
    Base.prototype.attachedComments = [];
...
// Placeholder fragments to be replaced by the source node’s compilation.
    ...
    isStatement(o) {
      return this.source.isStatement(o);// Update the target fragments with the result of compiling the source.
...
// presentational options).
    ...
    compileClosure(o) {
      return this.compileToFragments(o);////## Block
...
// Is this an empty block of code?
      ...
      jumps(o) {
        ...
        for (j = 0, len1 = ref1.length; j < len1; j++) {
          exp = ref1[j];
          if (jumpNode = exp.jumps(o)) {// A Block node does not return its entire body, rather it
...
// A **Block** is the only node that can serve as the root.
          ...
          return this.compileRoot(o);// Compile all expressions within the **Block** body. If we need to return
...
// We want to compile this and ignore the result.
            ...
            if (!node.isStatement(o)) {
              ...
              if (!(lastFragment.code === '' || lastFragment.suppressTrailingSemicolon)) {
...
// in the first place, but for now, clean up obvious double-parentheses.
      ...
      compileComments(fragments) {
        var code, commentFragment, fragment, fragmentIndex, j, k, l, len1, len2, newLineIndex, pastFragment, pastFragmentIndex, ref1, ref2, upcomingFragment, upcomingFragmentIndex;
...
// inside a string.
                if (pastFragmentIndex === 0) {
                  pastFragment.code = '\n' + pastFragment.code;
                  newLineIndex = 0;
                } else if (pastFragment.type === 'StringWithInterpolations' && pastFragment.code === '{') {
                  code = code.slice(1) + '\n'; // Move newline to end.
                  newLineIndex = 1;
                } else {
                  continue;
...
// impractical to abstract into a function that both blocks can share.
          if (fragment.trailingComments) {
            code = ((function() {
              ...
              ref2 = fragment.trailingComments;
              results = [];
            ...
            if (fragment.trailingComments[0].trail) {
              code = ` ${code}`;
            }
            ref2 = fragments.slice(fragmentIndex);
            for (upcomingFragmentIndex = l = 0, len2 = ref2.length; l < len2; upcomingFragmentIndex = ++l) {
              upcomingFragment = ref2[upcomingFragmentIndex];
...
// inside a string.
                ...
                } else if (upcomingFragment.type === 'StringWithInterpolations' && upcomingFragment.code === '}') {
                  code = `\n${code}\n`;
                  newLineIndex = 0;
                } else {
                  continue;
...
// A `return` is a *pureStatement*—wrapping it in a closure wouldn’t make sense.
  exports.Return = Return = (function() {
    class Return extends Base {
      ...
      compileNode(o) {
        var answer, fragment, fragments, i, j, len1;
        answer = [];
// TODO: If we call `expression.compile()` here twice, we’ll sometimes
// get back different results!
        if (this.expression) {
          fragments = this.expression.compileToFragments(o, LEVEL_PAREN);
          for (i = j = 0, len1 = fragments.length; j < len1; i = ++j) {
            fragment = fragments[i];
            if ((fragment != null ? fragment.type : void 0) === 'HereComment' && fragment.multiline) {
...
// into a generator.
  ...
  exports.AwaitReturn = AwaitReturn = class AwaitReturn extends Return {
    compileNode(o) {
      ...
      return super.compileNode(o);////## Value
  ...
  exports.Super = Super = (function() {
    class Super extends Base {
      ...
      compileNode(o) {
        ...
        return (new Value(new Literal('super'), this.accessor ? [this.accessor] : [])).compileToFragments(o);////## RegexWithInterpolations
...
// Hooks one constructor into another's prototype chain.
      compileToFragments(o) {
        return new Call(new Value(new Literal(utility('extend', o))), [this.child, this.parent]).compileToFragments(o);////## Access
...
// Store object spreads.
        ...
        return (new Call(new Literal('Object.assign'), slices)).compileToFragments(o);////## Arr
// An array literal.
      }
  ...
  exports.ExecutableClassBody = ExecutableClassBody = (function() {
    class ExecutableClassBody extends Base {
      ...
      compileNode(o) {
        ...
        return klass.compileToFragments(o);// Traverse the class's children and:
...
// Cache the value for reuse with rest elements
        [this.value, valueRef] = this.value.cache(o);// Find all rest elements.
...
// A shorthand `{a, b, @c} = val` pattern-match.
      ...
      compileConditional(o) {
        ...
        [left, right] = this.variable.cacheReference(o);// Disallow conditional assignment of undefined variables.
      ...
      compileSpecialMath(o) {
        ...
        return new Assign(left, new Op(this.context.slice(0, -1), right, this.value)).compileToFragments(o);// Compile the assignment from an array splice literal, using JavaScript's
// `Array#splice` method.
      }
...
// to that name.
      eachName(iterator, name = this.name) {
        ...
        atParam = (obj) => {
          return iterator(`@${obj.properties[0].name.value}`, obj, this);// * simple literals `foo`
        };
...
// as the chained expression is wrapped.
      ...
      compileExistence(o, checkOnlyUndefined) {
        ...
        return new If(new Existence(fst, checkOnlyUndefined), ref, {
          type: 'if'
        }).addElse(this.second).compileToFragments(o);// Compile a unary **Op**.
      }
...
// _the_ custom logic to output interpolated strings as code.
      ...
      compileNode(o) {
        ...
        if (this.csxAttribute) {
          ...
          return wrapped.compileNode(o);// Assumes that `expr` is `Value` » `StringLiteral` or `Op`
        }
        expr = this.body.unwrap();
        elements = [];
