...
// multiline need to be indented.
    ...
    class Code extends Base {
      constructor(params, body, funcGlyph, paramStart) {
        ...
        this.body.traverseChildren(false, (node) => {
          ...
          if (node instanceof For && node.isAsync()) {
            return false;
          }
        });
        this.propagateLhs();
  ...
  exports.For = For = (function() {
    class For extends While {
      ...
      isAwait() {
        var ref1;
        return (ref1 = this.await && !this.iter) != null ? ref1 : false;
      }
      ...
      isAsync() {
        ...
        isAsync = false;
        this.body.traverseChildren(false, (node) => {
          if ((node instanceof Op || node instanceof For) && node.isAwait()) {
            isAsync = true;
          }
      ...
      addSource(source) {
        ...
        attribs = ["name", "index", "guard", "step", "own", "ownTag", "await", "iter", "iterTag", "awaitTag", "object", "from"];
      ...
      // some cannot.
      compileNode(o) {
        var body, bodyFragments, compare, compareDown, declare, declareDown, defPart, down, forClose, forCode, forPartFragments, fragments, guardPart, idt1, idt2, increment, index, ivar, kvar, kvarAssign, last, lvar, name, namePart, ref, ref1, resultPart, returnResult, rvar, scope, source, step, stepNum, stepVar, svar, varPart, wrapperEnd, wrapperStart;
        body = Block.wrap([this.body]);
        ref1 = body.expressions, [last] = slice1.call(ref1, -1);
        if ((last != null ? last.jumps() : void 0) instanceof Return) {
          this.returns = false;
        }
        if (this.iter && !this.returns) {
          this.iterTag.error('cannot use lazy comprehensions as statements');
        }
        ...
        guardPart = '';
        defPart = '';
        idt1 = this.iter ? this.tab + TAB : this.tab;
        idt2 = idt1 + TAB;
        if (this.range) {
        ...
        if (this.object) {
          ...
          if (this.own) {
            guardPart = `\n${idt2}if (!${utility('hasProp', o)}.call(${svar}, ${kvar})) continue;`;
          }
        } else if (this.from) {
          if (this.await) {
        ...
        forClose = this.await ? '' : ')';
        fragments = fragments.concat(this.makeCode(idt1), this.makeCode(forCode), forPartFragments, this.makeCode(`${forClose} {${guardPart}${varPart}`), bodyFragments, this.makeCode(idt1), this.makeCode('}'));
        if (returnResult) {
          fragments.push(this.makeCode(returnResult));
        }
        if (this.iter) {
          wrapperStart = this.tab + `return (${this.await || this.isAsync() ? 'async ' : ''}function*() {\n`;
          wrapperEnd = `\n${this.tab}}).call(this)`;
          fragments = [].concat(this.makeCode(wrapperStart), fragments, this.makeCode(wrapperEnd));
        }
        return fragments;
      }
