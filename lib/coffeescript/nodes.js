...
(function() {
  ...
  exports.Range = Range = (function() {
    class Range extends Base {
      ...
      compileNode(o) {
        var cond, condPart, from, gt, idx, idxName, known, lowerBound, lt, namedIndex, stepCond, stepPart, to, upperBound, validStep, varPart;
        if (!this.fromVar) {
        ...
        if (this.step) {
          ...
          validStep = `(Number(${this.stepVar}) || (${this.stepVar}|0))`;
        }
...
this.toVar}`, upperBound = `${this.fromVar} >= ${idx} && ${gt} ${this.toVar}`, `${stepCond} && (${this.fromVar} <= ${this.toVar} ? ${lowerBound} : ${upperBound})`);
        ...
        // Generate the step.
        stepPart = this.stepVar ? `${idx} += ${validStep}` : known ? namedIndex ? from <= to ? `++${idx}` : `--${idx}` : from <= to ? `${idx}++` : `${idx}--` : namedIndex ? `${cond} ? ++${idx} : --${idx}` : `${cond} ? ${idx}++ : ${idx}--`;
        if (namedIndex) {
