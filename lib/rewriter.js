(function() {
  ...
  exports.Rewriter = function() {
    ...
    Rewriter.prototype.isUnaryStar = function(token, i) {
      var _ref;
      if (token[0] === 'MATH' && token[1] === '*' && this.tokens[i - 1][0] !== 'IDENTIFIER' && this.tokens[i - 1][0] !== 'NUMBER' && (_ref = this.tokens[i - 1][0], __indexOf.call(EXPRESSION_END, _ref) < 0)) {
        return true;
      }
      return false;
    };
    Rewriter.prototype.withTopLevelTokens = function(tokens, condition, action) {
      var i, level, token, _len, _results;
      level = 0;
      _results = [];
      for (i = 0, _len = tokens.length; i < _len; i++) {
        token = tokens[i];
        _results.push(token !== void 0 ? (token[0] === 'INDENT' ? level++ : void 0, token[0] === 'OUTDENT' ? level-- : void 0, level === 0 && condition(token, i, tokens) ? action(token, i, tokens) : void 0) : void 0);
      }
      return _results;
    };
    Rewriter.prototype.ensureArrayShorthandSyntax = function(tokens) {
      var condition;
      condition = function(token, i, tokens) {
        return token[0] === 'TERMINATOR' && tokens[i + 1] !== void 0 && (tokens[i + 1][0] !== 'MATH' || tokens[i + 1][1] !== '*');
      };
      return this.withTopLevelTokens(tokens, condition, function(token, i, tokens) {
        throw Error(" no * in array shorthand on line " + (token[2] + 1));
      });
    };
    Rewriter.prototype.rewriteArrayShorthand = function() {
      return this.scanTokens(function(token, i, tokens) {
        var begin, end, endCondition, idx, level;
        if (this.isUnaryStar(token, i)) {
          endCondition = function(token, i) {
            var _ref;
            return _ref = token[0], __indexOf.call(EXPRESSION_END, _ref) >= 0;
          };
          end = this.detectEnd(i, endCondition, function(token, i) {
            return i;
          });
          begin = i;
          this.ensureArrayShorthandSyntax(this.tokens.slice(begin, end + 1));
          end -= 1;
          tokens.splice(end, 0, [']', ']', tokens[end][2]]);
          tokens.splice(begin, 0, ['[', '[', tokens[begin][2]]);
          level = 0;
          for (idx = end; (end <= begin ? idx <= begin : idx >= begin); (end <= begin ? idx += 1 : idx -= 1)) {
            if (tokens[idx][0] === 'INDENT') {
              level++;
            }
            if (tokens[idx][0] === 'OUTDENT') {
              level--;
            }
            if (level === 0 && tokens[idx][0] === 'MATH' && tokens[idx][1] === '*') {
              tokens[idx] = void 0;
            }
          }
          idx = 0;
          while (idx < tokens.length) {
            if (tokens[idx] === void 0) {
              tokens.splice(idx, 1);
            } else {
              idx++;
            }
