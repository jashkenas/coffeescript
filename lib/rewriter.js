    ...
    Rewriter.prototype.adjustComments = function() {
      return this.scanTokens((function(__this) {
        return function(prev, token, post, i) {
          ...
          _c = [__this.tokens[i - 2], __this.tokens[i + 2]];
          before = _c[0];
          after = _c[1];
          if (after && after[0] === 'INDENT') {
            __this.tokens.splice(i + 2, 1);
            before && before[0] === 'OUTDENT' && post && (prev[0] === post[0]) && (post[0] === 'TERMINATOR') ? __this.tokens.splice(i - 2, 1) : __this.tokens.splice(i, 0, after);
          } else if (prev && !('TERMINATOR' === (_d = prev[0]) || 'INDENT' === _d || 'OUTDENT' === _d)) {
            if (post && post[0] === 'TERMINATOR' && after && after[0] === 'OUTDENT') {
              __this.tokens.splice.apply(__this.tokens, [i + 2, 0].concat(__this.tokens.splice(i, 2)));
              __this.tokens[i + 2][0] !== 'TERMINATOR' ? __this.tokens.splice(i + 2, 0, ['TERMINATOR', "\n", prev[2]]) : null;
            } else {
              __this.tokens.splice(i, 0, ['TERMINATOR', "\n", prev[2]]);
            }
            return 2;
          }
          return 1;
      ...
      closeCalls = (function(__this) {
        return function(i) {
          ...
          for (tmp = 0; (0 <= _c ? tmp < _c : tmp > _c); (0 <= _c ? tmp += 1 : tmp -= 1)) {
            __this.tokens.splice(i, 0, ['CALL_END', ')', __this.tokens[i][2]]);
          }
      ...
      return this.scanTokens((function(__this) {
        return function(prev, token, post, i) {
          ...
          if (prev && prev.spaced && include(IMPLICIT_FUNC, prev[0]) && include(IMPLICIT_CALL, tag) && !(tag === '!' && (('IN' === (_c = post[0]) || 'OF' === _c)))) {
            __this.tokens.splice(i, 0, ['CALL_START', '(', token[2]]);
          ...
          if (open && !token.generated && prev[0] !== ',' && (!post || include(IMPLICIT_END, tag))) {
            j = 1;
            while ((typeof (_d = (nx = __this.tokens[i + j])) !== "undefined" && _d !== null) && include(IMPLICIT_END, nx[0])) {
              j++;
            }
            if ((typeof nx !== "undefined" && nx !== null) && nx[0] === ',' && __this.tokens[i + j - 1][0] === 'OUTDENT') {
              if (tag === 'TERMINATOR') {
                __this.tokens.splice(i, 1);
              }
            } else {
      ...
      return this.scanTokens((function(__this) {
        return function(prev, token, post, i) {
          ...
          if (token[0] === 'ELSE' && prev[0] !== 'OUTDENT') {
            __this.tokens.splice.apply(__this.tokens, [i, 0].concat(__this.indentation(token)));
            return 2;
          }
          if (token[0] === 'CATCH' && (__this.tokens[i + 2][0] === 'TERMINATOR' || __this.tokens[i + 2][0] === 'FINALLY')) {
            __this.tokens.splice.apply(__this.tokens, [i + 2, 0].concat(__this.indentation(token)));
            return 4;
          }
          ...
          starter = token[0];
          _c = __this.indentation(token);
          indent = _c[0];
          outdent = _c[1];
          indent.generated = (outdent.generated = true);
          __this.tokens.splice(i + 1, 0, indent);
          idx = i + 1;
          parens = 0;
          while (true) {
            idx += 1;
            tok = __this.tokens[idx];
            pre = __this.tokens[idx - 1];
            if ((!tok || (include(SINGLE_CLOSERS, tok[0]) && tok[1] !== ';' && parens === 0) || (tok[0] === ')' && parens === 0)) && !(tok[0] === 'ELSE' && !('IF' === starter || 'THEN' === starter))) {
              ...
              __this.tokens.splice(insertion, 0, outdent);
              break;
            }
            if (tok[0] === '(') {
              parens += 1;
        ...
        return function(prev, token, post, i) {
          ...
          if (include(EXPRESSION_START, tag)) {
            ...
            if (debt[inv] > 0) {
              ...
              if ((__this.tokens[i + 2] == undefined ? undefined : __this.tokens[i + 2][0]) === mtag) {
                __this.tokens.splice(i + 3, 0, val);
                stack.push(match);
              } else {
                __this.tokens.splice(i, 0, val);
              }
              return 1;
            }
          } else {
