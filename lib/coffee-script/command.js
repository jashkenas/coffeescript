(function() {
  ...
  SWITCHES = [['-b', '--bare', 'compile without the top-level function wrapper'], ['-c', '--compile', 'compile to JavaScript and save as .js files'], ['-e', '--eval', 'compile a string from the command line'], ['-h', '--help', 'display this help message'], ['-i', '--interactive', 'run an interactive CoffeeScript REPL'], ['-j', '--join [FILE]', 'concatenate the scripts before compiling'], ['-l', '--lint', 'pipe the compiled JavaScript through JavaScript Lint'], ['-n', '--nodes', 'print the parse tree that Jison produces'], ['--nodejs [ARGS]', 'pass options through to the "node" binary'], ['-o', '--output [DIR]', 'set the directory for compiled JavaScript'], ['-p', '--print', 'print the compiled JavaScript to stdout'], ['-r', '--require [FILE*]', 'require a library before executing your script'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['-t', '--tokens', 'print the tokens that the lexer produces'], ['-v', '--version', 'display CoffeeScript version'], ['-w', '--watch', 'watch scripts for changes and rerun commands']];
  opts = {};
  sources = [];
  contents = [];
  optionParser = null;
