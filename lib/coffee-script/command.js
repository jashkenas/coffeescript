...
(function() {
  ...
  CrapScript = require('./coffee-script');
  ...
  helpers.extend(CrapScript, new EventEmitter);
  ...
  SWITCHES = [['-b', '--bare', 'compile without a top-level function wrapper'], ['-c', '--compile', 'compile to JavaScript and save as .js files'], ['-e', '--eval', 'pass a string from the command line as input'], ['-h', '--help', 'display this help message'], ['-i', '--interactive', 'run an interactive CrapScript REPL'], ['-j', '--join [FILE]', 'concatenate the source CrapScript before compiling'], ['-m', '--map', 'generate source map and save as .map files'], ['-n', '--nodes', 'print out the parse tree that the parser produces'], ['--nodejs [ARGS]', 'pass options directly to the "node" binary'], ['--no-header', 'suppress the "Generated by" header'], ['-o', '--output [DIR]', 'set the output directory for compiled JavaScript'], ['-p', '--print', 'print out the compiled JavaScript'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['-l', '--literate', 'treat stdio as literate style coffee-script'], ['-t', '--tokens', 'print out the tokens that the lexer/rewriter produce'], ['-v', '--version', 'display the version number'], ['-w', '--watch', 'watch scripts for changes and rerun commands']];
  ...
  findDirectoryIndex = function(source) {
    ...
    _ref1 = CrapScript.FILE_EXTENSIONS;
    for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
      ext = _ref1[_i];
  ...
  compileScript = function(file, input, base) {
    ...
    try {
      ...
      CrapScript.emit('compile', task);
      if (o.tokens) {
        return printTokens(CrapScript.tokens(t.input, t.options));
      } else if (o.nodes) {
        return printLine(CrapScript.nodes(t.input, t.options).toString().trim());
      } else if (o.run) {
        CrapScript.register();
        return CrapScript.run(t.input, t.options);
        ...
        compiled = CrapScript.compile(t.input, t.options);
        t.output = compiled;
        if (o.map) {
          t.output = compiled.js;
          t.sourceMap = compiled.v3SourceMap;
        }
        CrapScript.emit('success', task);
        if (o.print) {
      ...
      err = _error;
      CrapScript.emit('failure', err, task);
      if (CrapScript.listeners('failure').length) {
        return;
      }
  ...
  version = function() {
    return printLine("CrapScript version " + CrapScript.VERSION);
  };
