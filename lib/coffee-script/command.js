...
(function() {
  ...
  SWITCHES = [['-b', '--bare', 'compile without a top-level function wrapper'], ['-c', '--compile', 'compile to JavaScript and save as .js files'], ['-e', '--eval', 'pass a string from the command line as input'], ['-h', '--help', 'display this help message'], ['-i', '--interactive', 'run an interactive CoffeeScript REPL'], ['-j', '--join [FILE]', 'concatenate the source CoffeeScript before compiling'], ['-m', '--map', 'generate source map and save as .js.map files'], [null, '--nodejsmap', 'add the entry point for node.js sourcemap'], ['-n', '--nodes', 'print out the parse tree that the parser produces'], ['--nodejs [ARGS]', 'pass options directly to the "node" binary'], ['--no-header', 'suppress the "Generated by" header'], ['-o', '--output [DIR]', 'set the output directory for compiled JavaScript'], ['-p', '--print', 'print out the compiled JavaScript'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['-l', '--literate', 'treat stdio as literate style coffee-script'], ['-t', '--tokens', 'print out the tokens that the lexer/rewriter produce'], ['-v', '--version', 'display the version number'], ['-w', '--watch', 'watch scripts for changes and rerun commands']];
  ...
  compileScript = function(file, input, base) {
    ...
    try {
      ...
      if (o.tokens) {
        ...
        if (o.nodejsmap) {
          t.sourceNodejsMap = true;
        }
        CoffeeScript.emit('success', task);
        if (o.print) {
          return printLine(t.output.trim());
        } else if (o.compile || o.map) {
          return writeJs(base, t.file, t.output, options.jsPath, t.sourceMap, t.sourceNodejsMap);
        }
      }
    } catch (_error) {
      err = _error;
  ...
  writeJs = function(base, sourcePath, js, jsPath, generatedSourceMap, generatedNodeSourceMap) {
    var compile, jsDir, sourceMapPath;
    if (generatedSourceMap == null) {
      generatedSourceMap = null;
    }
    if (generatedNodeSourceMap == null) {
      generatedNodeSourceMap = null;
    }
    ...
    compile = function() {
      if (opts.compile) {
        ...
        if (generatedNodeSourceMap) {
          js = "//# sourceMappingURL=" + (helpers.baseFileName(sourceMapPath, false, useWinPathSep)) + "\nrequire('source-map-support').install();\n" + js + "\n";
        }
