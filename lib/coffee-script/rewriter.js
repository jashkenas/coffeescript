...
(function() {
  var BALANCED_PAIRS, CONDITIONAL_OPERATIONS, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_BLOCK, IMPLICIT_CALL, IMPLICIT_END, IMPLICIT_FUNC, IMPLICIT_UNSPACED_CALL, INVERSES, LINEBREAKS, SINGLE_CLOSERS, SINGLE_LINERS, left, rite, _i, _len, _ref,
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.addImplicitParentheses = function() {
      var action, callIndex, condition, insideOpExpr, noCall, seenControl, seenSingle;
      noCall = seenSingle = seenControl = insideOpExpr = false;
      callIndex = null;
      condition = function(token, i) {
        ...
        return !token.generated && this.tag(i - 1) !== ',' && (__indexOf.call(IMPLICIT_END, tag) >= 0 || (tag === 'INDENT' && !seenControl)) && (tag !== 'INDENT' || (((_ref = this.tag(i - 2)) !== 'CLASS' && _ref !== 'EXTENDS') && (_ref1 = this.tag(i - 1), __indexOf.call(IMPLICIT_BLOCK, _ref1) < 0) && !((!insideOpExpr || callIndex === i - 1) && (post = this.tokens[i + 1]) && post.generated && post[0] === '{')));
      };
      action = function(token, i) {
      ...
      return this.scanTokens(function(token, i, tokens) {
        ...
        if (__indexOf.call(CONDITIONAL_OPERATIONS, tag) >= 0) {
          insideOpExpr = true;
        }
        if (tag === 'INDENT') {
          insideOpExpr = false;
