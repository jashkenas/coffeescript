  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.addImplicitBracesAndParentheses = function() {
      ...
      detectEndBrace = function(token, i, _arg) {
        var embeddedEnd, followedByObjAssign, implicitEnd, line, paramEnd, sameLine, simpleEnd, startsLine, tag, value,
          _this = this;
        sameLine = _arg.sameLine, startsLine = _arg.startsLine;
        if ('HERECOMMENT' === this.tag(i + 1)) return 1;
        tag = token[0], value = token[1], line = token[2];
        embeddedEnd = tag === 'TERMINATOR' && !startsLine && this.tag(i - 1) !== ',';
        implicitEnd = __indexOf.call(IMPLICIT_END, tag) >= 0 && sameLine;
        followedByObjAssign = (function(i) {
          if (_this.tag(i + 1) === 'TERMINATOR') i += 1;
          if (_this.tag(i + 1) === '@') i += 1;
          return _this.tag(i + 2) === ':';
        })(i);
        simpleEnd = tag === 'TERMINATOR' && !followedByObjAssign;
        paramEnd = tag === ',' && this.tag(i + 1) !== 'OUTDENT' && !followedByObjAssign;
        if (embeddedEnd || implicitEnd || simpleEnd || paramEnd) throw 'STOP';
        return 1;
      };
