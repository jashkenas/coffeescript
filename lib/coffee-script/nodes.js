...
(function() {
    ...
    Class.prototype.compileNode = function(o) {
      ...
      if (this.variable) {
        klass = new Assign(this.variable, klass, null, {
          moduleStatement: this.moduleStatement
        });
      }
  ...
  exports.Module = Module = (function(superClass1) {
    extend1(Module, superClass1);

    function Module(type1, clause, moduleName, _default) {
      var ref3;
      this.type = type1;
      this.clause = clause;
      this.moduleName = moduleName;
      this["default"] = _default != null ? _default : false;
      if ((ref3 = this.type) !== 'import' && ref3 !== 'export') {
        this.error('module type must be import or export');
      }
      if ((this.moduleName != null) && this.moduleName instanceof StringWithInterpolations) {
        this.moduleName.error('the name of the module to be imported from must be an uninterpolated string');
      }
      if (this.type === 'export' && this["default"] === false && this.clause instanceof Class && (this.clause.variable == null)) {
        this.clause.error('anonymous classes cannot be exported');
      }
    }

    Module.prototype.children = ['clause', 'moduleName'];

    Module.prototype.isStatement = YES;

    Module.prototype.jumps = THIS;

    Module.prototype.makeReturn = THIS;

    Module.prototype.compileNode = function(o) {
      var code, ref3;
      if (o.indent.length !== 0) {
        this.error(this.type + " statements must be at top-level scope");
      }
      code = [];
      code.push(this.makeCode("" + this.tab + this.type + " "));
      if (this["default"]) {
        code.push(this.makeCode('default '));
      }
      if ((this.clause != null) && this.clause.length !== 0) {
        if (this["default"] === false && (this.clause instanceof Assign || this.clause instanceof Class)) {
          code.push(this.makeCode('var '));
          this.clause.moduleStatement = this.type;
        }
      ...
      if (((ref3 = this.moduleName) != null ? ref3.value : void 0) != null) {
        if (!(this.type === 'import' && this.clause === null)) {
          code.push(this.makeCode(' from '));
        }
        code.push(this.makeCode(this.moduleName.value));
      }
  ...
  exports.Import = Import = (function(superClass1) {
    extend1(Import, superClass1);

    function Import(clause, moduleName) {
      this.clause = clause;
      this.moduleName = moduleName;
      Import.__super__.constructor.call(this, 'import', this.clause, this.moduleName, false);
    }
  ...
  exports.Export = Export = (function(superClass1) {
    extend1(Export, superClass1);

    function Export(clause, moduleName, _default) {
      this.clause = clause;
      this.moduleName = moduleName;
      this["default"] = _default != null ? _default : false;
      Export.__super__.constructor.call(this, 'export', this.clause, this.moduleName, this["default"]);
    }
  ...
  exports.ExportDefault = ExportDefault = (function(superClass1) {
    extend1(ExportDefault, superClass1);

    function ExportDefault(clause) {
      this.clause = clause;
      ExportDefault.__super__.constructor.call(this, 'export', this.clause, null, true);
    }
  ...
  exports.ExportImport = ExportImport = (function(superClass1) {
    extend1(ExportImport, superClass1);

    function ExportImport(clause, moduleName) {
      this.clause = clause;
      this.moduleName = moduleName;
      if (this.clause instanceof Literal && this.clause.value === '*' && (this.moduleName == null)) {
        this.clause.error('missing module name to export * from');
      }
      ExportImport.__super__.constructor.call(this, 'export', this.clause, this.moduleName, false);
    }
  ...
  exports.ModuleList = ModuleList = (function(superClass1) {
    extend1(ModuleList, superClass1);

    function ModuleList(firstIdentifiers, firstWrapped, secondIdentifiers, secondWrapped) {
      this.firstIdentifiers = firstIdentifiers != null ? firstIdentifiers : [];
      this.firstWrapped = firstWrapped != null ? firstWrapped : false;
      this.secondIdentifiers = secondIdentifiers != null ? secondIdentifiers : [];
      this.secondWrapped = secondWrapped != null ? secondWrapped : false;
    }

    ModuleList.prototype.children = ['firstIdentifiers', 'secondIdentifiers'];

    ModuleList.prototype.compileNode = function(o) {
      var code;
      if (!this.firstIdentifiers.length) {
        return [];
      }
      if (this.firstWrapped === false && this.firstIdentifiers.length > 1) {
        if (this.firstIdentifiers.length === 2) {
          if (!this.firstIdentifiers[1].originalIsAll) {
            this.firstIdentifiers[1].error('unless wrapped in curly braces, a second imported member can only be of the form: * as name');
          }
        } else {
          this.firstIdentifiers[2].error('unless wrapped in curly braces, no more than two members can be imported');
        }
      }
      code = [];
      o.indent += TAB;
      code = code.concat(this.compileIdentifiers(o, this.firstIdentifiers, this.firstWrapped));
      if (this.secondIdentifiers.length) {
        code.push(this.makeCode(', '));
        code = code.concat(this.compileIdentifiers(o, this.secondIdentifiers, this.secondWrapped));
      }
      return code;
    };

    ModuleList.prototype.compileIdentifiers = function(o, identifiers, wrapped) {
      var code, compiledList, fragments, identifier, index, j, len1;
      o.wrapped = wrapped;
      compiledList = (function() {
        var j, len1, results;
        results = [];
        for (j = 0, len1 = identifiers.length; j < len1; j++) {
          identifier = identifiers[j];
          results.push(identifier.compileToFragments(o, LEVEL_LIST));
        }
        return results;
      })();
      code = [];
  ...
  exports.ModuleIdentifier = ModuleIdentifier = (function(superClass1) {
    extend1(ModuleIdentifier, superClass1);

    function ModuleIdentifier(original, alias, originalIsAll, aliasIsDefault) {
      this.original = original;
      this.alias = alias;
      this.originalIsAll = originalIsAll != null ? originalIsAll : false;
      this.aliasIsDefault = aliasIsDefault != null ? aliasIsDefault : false;
    }

    ModuleIdentifier.prototype.children = ['original', 'alias'];

    ModuleIdentifier.prototype.compileNode = function(o) {
      var code, variableName;
      if (o.wrapped === false && (this.alias != null) && this.originalIsAll === false) {
        this.original.error('unless enclosed by curly braces, only * can be aliased');
      }
      variableName = this.alias != null ? this.alias.value : this.original.value;
  ...
  exports.Assign = Assign = (function(superClass1) {
    ...
    function Assign(variable1, value1, context, options) {
      ...
      this.param = options.param, this.subpattern = options.subpattern, this.operatorToken = options.operatorToken, this.moduleStatement = options.moduleStatement;
    }

    Assign.prototype.children = ['variable', 'value'];

    Assign.prototype.isStatement = function(o) {
      return (o != null ? o.level : void 0) === LEVEL_TOP && (this.context != null) && (this.moduleStatement || indexOf.call(this.context, "?") >= 0);
    };
    ...
    Assign.prototype.compileNode = function(o) {
      ...
      if (!this.context) {
        ...
        if (!(typeof varBase.hasProperties === "function" ? varBase.hasProperties() : void 0)) {
          if (this.moduleStatement) {
            o.scope.add(varBase.value, this.moduleStatement);
          } else if (this.param) {
