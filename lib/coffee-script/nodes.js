...
(function() {
  ...
  exports.Class = Class = (function(superClass1) {
    ...
    Class.prototype.overloadMethodName = function(method_name, property_name, is_static) {
      var prefix;
      prefix = "__" + method_name;
      if (is_static) {
        return prefix + "_static_" + property_name;
      } else {
        return prefix + "_" + property_name;
      }
    };

    Class.prototype.makePropertyOverload = function(assignment) {
      var func, is_getter, is_static, match, method_name, o, property_name, safe_name, variable;
      func = assignment.value;
      variable = assignment.variable;
      is_getter = func.params.length === 0;
      is_static = assignment.variable["this"] === true;
      method_name = is_getter ? "get" : "set";
      if (is_static) {
      ...
      match = ((function() {
        ...
        for (j = 0, len1 = ref3.length; j < len1; j++) {
          o = ref3[j];
          if (o.property_name === property_name && o["static"] === is_static) {
            results.push(o);
          }
        }
        return results;
      }).call(this))[0];
      safe_name = this.overloadMethodName(method_name, property_name, is_static);
      if (match) {
        ...
        match = {
          ...
          overloads: [method_name]
        };
        this.overloads.push(match);
      }
      return new Assign(new IdentifierLiteral(safe_name), func);
    };

    Class.prototype.addOverloads = function(o) {
      var access, fn_name, is_static, j, len1, mappings, overload, property_name, ref3, target, value;
      ref3 = this.overloads;
      for (j = 0, len1 = ref3.length; j < len1; j++) {
        overload = ref3[j];
        ...
        mappings = [];
        if (is_static) {
          target = new Literal(this.determineName());
        } else {
          access = [new Access(new PropertyName("prototype"))];
          target = new Value(new Literal(this.determineName()), access);
        }
        if ((overload.overloads.indexOf("get")) !== -1) {
          fn_name = this.overloadMethodName("get", property_name, is_static);
          mappings.push(new Assign(new Literal("get"), new Literal(fn_name), "object"));
        }
        if ((overload.overloads.indexOf("set")) !== -1) {
          fn_name = this.overloadMethodName("set", property_name, is_static);
          mappings.push(new Assign(new Literal("set"), new Literal(fn_name), "object"));
        }
        value = new Value(new Literal("Object"), [new Call(new Access(new PropertyName("defineProperty")), [target, new Literal("\"" + property_name + "\""), new Obj(mappings)])]);
        this.body.expressions.push(value);
      }
      return true;
    };

    Class.prototype.addProperties = function(node, name, o) {
      var acc, assign, base, exprs, func, overloads, props;
      ...
      exprs = (function() {
        ...
        while (assign = props.shift()) {
          if (assign instanceof Assign) {
            ...
            func = assign.value;
            if (func.overload && func instanceof Code) {
              assign = this.makePropertyOverload(assign);
    ...
    Class.prototype.compileNode = function(o) {
      var args, argumentsNode, func, jumpNode, klass, lname, name, overloads, ref3, superClass;
