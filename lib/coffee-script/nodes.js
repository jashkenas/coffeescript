...
(function() {
  var Access, Arr, Assign, Base, Block, Call, Class, Closure, Code, CodeFragment, Comment, Existence, Extends, For, IDENTIFIER, IDENTIFIER_STR, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, METHOD_DEF, NEGATE, NO, Obj, Op, Param, Parens, RESERVED, Range, Return, SIMPLENUM, STRICT_PROSCRIBED, Scope, Slice, Splat, Switch, TAB, THIS, Throw, Try, UTILITIES, Value, While, YES, addLocationDataFn, compact, del, ends, extend, flatten, fragmentsToText, last, locationDataToString, merge, multident, some, starts, throwSyntaxError, unfoldSoak, utility, _ref, _ref1, _ref2, _ref3,
    ...
    __hasES5Properties = (function () { if (!__defineProp || !__getProp || !__hasProp) return false; try { __defineProp ({}, 'test', {}); __getProp ({foo:5}, 'foo'); return true; } catch (e) { return false; } })(),
    __copyProp = function(child, parent, key) { var desc = __getProp(parent, key); __defineProp (child, key, desc); },
    __extends = function(child, parent) { for (var key in parent) { if (__hasES5Properties) { if (__hasProp.call(parent, key)) __copyProp(child, parent, key); } else child[key] = parent[key]; function ctor() { this.constructor = child; } } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  ...
  UTILITIES = {
    ...
    hasES5Properties: function() {
      return "(function () { if (!" + (utility('defineProp')) + " || !" + (utility('getProp')) + " || !" + (utility('hasProp')) + ") return false; try { " + (utility('defineProp')) + " ({}, 'test', {}); " + (utility('getProp')) + " ({foo:5}, 'foo'); return true; } catch (e) { return false; } })()";
    },
    copyProp: function() {
