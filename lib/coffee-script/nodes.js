...
(function() {
  ...
  exports.Code = Code = (function(superClass1) {
    ...
    Code.prototype.compileNode = function(o) {
      var answer, code, expansionParamName, exprs, i, j, k, l, len1, len2, len3, len4, len5, lit, m, p, param, params, ref, ref3, ref4, ref5, ref6, ref7, ref8, splatIndex, splatNotLast, splatParam, splatParamReference, uniqs, val, wasEmpty;
      if (this.bound) {
      ...
      for (i = j = 0, len1 = ref4.length; j < len1; i = ++j) {
        ...
        if (param instanceof Expansion) {
          expansionParamName = o.scope.freeVariable('args');
        }
        splatIndex = i;
        splatNotLast = i !== this.params.length - 1;
        break;
      }
      ref5 = this.params;
      for (i = k = 0, len2 = ref5.length; k < len2; i = ++k) {
        ...
        if (!param.splat) {
          if (splatParam instanceof Expansion && i > splatIndex) {
            exprs.push(new Assign(param.name, new Value(new IdentifierLiteral(expansionParamName), [new Index(new Op('-', new Value(new IdentifierLiteral(expansionParamName), [new Access(new PropertyName('length'))]), new NumberLiteral(this.params.length - i)))])));
            if (param.value) {
      ...
      if (splatParamReference != null) {
        params.push(splatParamReference);
      } else if (splatParam != null) {
      ...
      for (i = l = 0, len3 = params.length; l < len3; i = ++l) {
        ...
        if ((splatIndex != null) && i === params.length - 1) {
          answer.push(this.makeCode('...'));
        }
        answer.push.apply(answer, param.compileToFragments(o));
      }
      answer.push(this.makeCode(!this.bound ? ') {' : ') => {'));
      if (splatNotLast && !(splatParam instanceof Expansion)) {
        answer.push(this.makeCode("\n" + o.indent + "["));
        ref7 = this.params;
        for (i = m = 0, len4 = ref7.length; m < len4; i = ++m) {
          param = ref7[i];
          if (!(i > splatIndex)) {
            continue;
          }
        ...
        answer.push(this.makeCode(", ..." + splatParam.name.value + "] = " + splatParam.name.value + ".slice(-" + (this.params.length - 1 - splatIndex) + ").concat(["));
        ref8 = this.params;
        for (i = p = 0, len5 = ref8.length; p < len5; i = ++p) {
          param = ref8[i];
          if (!(i > splatIndex)) {
            continue;
          }
          ...
          answer.push(this.makeCode(param.name.value));
        }
        answer.push(this.makeCode("], " + splatParam.name.value + ".slice(0, -" + (this.params.length - 1 - splatIndex) + "));\n"));
      }
