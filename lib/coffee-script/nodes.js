...
(function() {
  ...
  exports.Code = Code = (function(superClass1) {
    ...
    Code.prototype.compileNode = function(o) {
      var answer, code, exprs, haveSplatParam, i, j, k, len1, len2, lit, param, paramNames, params, paramsAfterSplat, ref, ref3, ref4, ref5, ref6, splatParamName, val, wasEmpty;
      if (this.bound) {
      ...
      for (i = j = 0, len1 = ref4.length; j < len1; i = ++j) {
        ...
        if (param.splat || param instanceof Expansion) {
          haveSplatParam = true;
          if (param.splat) {
            if (((ref5 = param.name) != null ? ref5["this"] : void 0) != null) {
              splatParamName = paramNames[i].replace(/@|this\.(.*)/, '$1');
              params.push(new Value(new IdentifierLiteral(splatParamName)));
              exprs.push(new Assign(new Value(param.name), new Value(new IdentifierLiteral(splatParamName)), '=', {
                param: true
              }));
            } else {
          ...
          if (!haveSplatParam) {
            ...
            exprs.push(new Assign(param.name, new Value(new Call(new Value(new IdentifierLiteral(splatParamName), [new Access(new PropertyName('slice'))]), [new NumberLiteral(i - this.params.length)]), [new Index(new NumberLiteral(0))])));
            if (param.value != null) {
              lit = new Literal(param.name.value + ' == null');
              ref = new Assign(new Value(param.name), param.value, '=');
              exprs.push(new If(lit, ref));
            }
            if (((ref6 = param.name) != null ? ref6["this"] : void 0) == null) {
              o.scope.add(param.name.value, 'var', true);
            }
          }
        }
      }
      if (paramsAfterSplat !== 0) {
        exprs.push(new Assign(new IdentifierLiteral(splatParamName), new Call(new Value(new IdentifierLiteral(splatParamName), [new Access(new PropertyName('slice'))]), [new NumberLiteral(0), new Op('-', new Value(new IdentifierLiteral(splatParamName), [new Access(new PropertyName('length'))]), new NumberLiteral(paramsAfterSplat))])));
      }
