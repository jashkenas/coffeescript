...
(function() {
  ...
  exports.JsxElement = JsxElement = (function(superClass1) {
    extend1(JsxElement, superClass1);

    function JsxElement(options) {
      var base1, base2, base3, ref3, ref4, ref5;
      this.name = options.name, this.children_ = (ref3 = options.children) != null ? ref3 : [], this.attributes = (ref4 = options.attributes) != null ? ref4 : {}, this.shorthands = (ref5 = options.shorthands) != null ? ref5 : {};
      if ((base1 = this.attributes).object == null) {
        base1.object = {};
      }
      if ((base2 = this.attributes).list == null) {
        base2.list = [];
      }
      if ((base3 = this.shorthands).classes == null) {
        base3.classes = [];
      }
    }

    JsxElement.prototype.compileNode = function(o) {
      var compiledChildren, compiledClassAttribute, compiledIdAttribute, compiledListAttributes, compiledObjectAttributes, endTag, isString, ref3, ref4, startTag;
      compiledIdAttribute = (function(_this) {
        return function() {
          if (!_this.shorthands.id) {
            return [];
          }
          return [_this.makeCode(" id='" + _this.shorthands.id + "'")];
        };
      })(this)();
      compiledClassAttribute = (function(_this) {
        return function() {
          if (!_this.shorthands.classes.length) {
            return [];
          }
          return [_this.makeCode(" className='" + (_this.shorthands.classes.join(' ')) + "'")];
        };
      })(this)();
      compiledListAttributes = (function(_this) {
        return function() {
          var attr, j, len1, name, ref3, ref4, value;
          if (!_this.attributes.list.length) {
            return [];
          }
          attr = [];
          ref3 = _this.attributes.list;
          for (j = 0, len1 = ref3.length; j < len1; j++) {
            ref4 = ref3[j], name = ref4.name, value = ref4.value;
            attr.push(_this.makeCode(' '));
            attr.push(_this.makeCode(name));
            attr.push(_this.makeCode('='));
            if (value instanceof StringLiteral) {
              attr.push.apply(attr, value.compileToFragments(o));
            } else {
              attr.push(_this.makeCode('{'));
              attr.push.apply(attr, value.compileToFragments(o));
              attr.push(_this.makeCode('}'));
      ...
      compiledObjectAttributes = (ref3 = (ref4 = this.attributes.object) != null ? typeof ref4.compileToFragments === "function" ? ref4.compileToFragments(o) : void 0 : void 0) != null ? ref3 : [];
      startTag = [this.makeCode('<'), this.makeCode(this.name)].concat(slice.call(compiledIdAttribute), slice.call(compiledClassAttribute), slice.call(compiledListAttributes), slice.call(compiledObjectAttributes), [this.makeCode('>')]);
      isString = function(obj) {
        return Object.prototype.toString.call(obj) === '[object String]';
      };
      compiledChildren = (function(_this) {
        return function() {
          var child, compiled, initialSpace, ref5;
          if (!_this.children_.length) {
            return [];
          }
          compiled = flatten((function() {
            var j, len1, ref5, results;
            ref5 = this.children_;
            results = [];
            for (j = 0, len1 = ref5.length; j < len1; j++) {
              child = ref5[j];
              results.push([this.makeCode(' ')].concat(slice.call((isString(child) ? [this.makeCode(child)] : child instanceof JsxElement ? child.compileToFragments(o) : [this.makeCode('{'), child.compileToFragments(o), this.makeCode('}')]))));
            }
            return results;
          }).call(_this));
          ref5 = compiled, initialSpace = ref5[0], compiled = 2 <= ref5.length ? slice.call(ref5, 1) : [];
          return compiled;
        };
      })(this)();
      endTag = [this.makeCode('</'), this.makeCode(this.name), this.makeCode('>')];
      return slice.call(startTag).concat(slice.call(compiledChildren), slice.call(endTag));
    };
  ...
  exports.JsxAttributesObj = JsxAttributesObj = (function(superClass1) {
    extend1(JsxAttributesObj, superClass1);

    function JsxAttributesObj() {
      return JsxAttributesObj.__super__.constructor.apply(this, arguments);
    }

    JsxAttributesObj.prototype.compileNode = function(o) {
      var answer, j, len1, prop, ref3;
      answer = [];
      ref3 = this.properties;
      for (j = 0, len1 = ref3.length; j < len1; j++) {
        prop = ref3[j];
        ...
        } else if (prop instanceof Value && prop["this"]) {
          prop = new Assign(prop.properties[0].name, prop, '=');
        } else {
          prop = new Assign(prop, prop, '=');
        }
        prop.jsxAttribute = true;
        answer.push.apply(answer, prop.compileToFragments(o, LEVEL_TOP));
      }
      return answer;
    };

    return JsxAttributesObj;
  ...
  exports.Assign = Assign = (function(superClass1) {
    ...
    Assign.prototype.compileNode = function(o) {
      ...
      if (this.jsxAttribute) {
        val = [this.makeCode('{')].concat(slice.call(val), [this.makeCode('}')]);
      }
      ...
      answer = compiledName.concat(this.makeCode("" + (this.jsxAttribute ? '' : ' ') + (this.context || '=') + (this.jsxAttribute ? '' : ' ')), val);
