...
(function() {
  var ACCESSOR, CoffeeScript, Module, REPL_PROMPT, REPL_PROMPT_CONTINUATION, REPL_PROMPT_MULTILINE, SIMPLEVAR, Script, autocomplete, backlog, child_process, completeAttribute, completeVariable, enableColours, error, fs, getCompletions, inspect, multilineMode, path, pipedInput, readline, repl, run, stdin, stdout;
  ...
  repl.input.on('keypress', function(char, key) {
    ...
    getTmpFile = function(cb) {
      var file, filePath, tmp, writeTemp;
      writeTemp = function(file, cb) {
        return fs.writeFile(file, backlog + repl.line, function() {
          return cb(file);
        });
      };
      ...
      try {
        return child_process.exec("mktemp --suffix=.coffee", function(err, stdout, stderr) {
          return writeTemp((err ? filePath : stdout), cb);
        });
      } catch (e) {
        return writeTemp(filePath, cb);
      }
    };
