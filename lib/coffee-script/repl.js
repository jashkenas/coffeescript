...
(function() {
  ...
  repl.input.on('keypress', function(char, key) {
    var editor, eso, file, filePath, pso, rplListeners, stdinListeners, tmp;
    ...
    return editor.on('exit', function(code) {
      ...
      for (_j = 0, _len1 = rplListeners.length; _j < _len1; _j++) {
        listener = rplListeners[_j];
        repl.input.on('keypress', listener);
      }
      if (code === 0) {
