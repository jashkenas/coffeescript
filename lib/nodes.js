(function() {
  ...
  exports.Obj = Obj = (function() {
    __extends(Obj, Base);
    function Obj(props, /*@*/generated/* = false*/) {
  ...
  exports.Class = Class = (function() {
    ...
    function Class(/*@*/variable, /*@*/parent, /*@*/body/* = new Block*/) {
  ...
  exports.Code = Code = (function() {
    ...
    Code.prototype.compileNode = function(o) {
      var argLengthVar, callArgs, code, exprs, i, idt, idx, lit, pName, param, pos, ref, splat, splatIndex, splatName, v, val, vars, vc, vvc, wasEmpty, _len, _len2, _ref2, _ref3, _ref4, _ref5, _ref6;
      ...
      for (idx = 0, _len = _ref2.length; idx < _len; idx++) {
        ...
        if (param.splat) {
          if (splat) {
            throw new Error('multiple splats in a single parameter list');
          }
          splat = param;
          splatIndex = idx;
        }
        ref = param;
        if (param.isComplex()) {
          this.params[idx] = val = ref = param.asReference(o);
          if (param.value) {
      ...
      if (splat) {
        if ((_ref3 = splat.name) != null ? _ref3["this"] : void 0) {
          splat = splat.name.properties[0];
        }
        splatName = ((_ref4 = splat.reference) != null ? _ref4.name : void 0) || splat.name;
        if (splat.name.objects == null) {
          o.scope.add(splatName.value, 'var', true);
        }
        argLengthVar = '_numArgs';
        o.scope.add(argLengthVar, 'var', true);
        exprs.unshift.apply(exprs, [
          new Assign(new Value(splatName), new Arr([])), new If(new Op('>', new Assign(new Value(new Literal(argLengthVar)), new Value(new Literal('arguments'), [new Access(new Literal('length'))])), new Value(new Literal(this.params.length - 1))), [(callArgs = [new Literal('arguments'), new Literal(splatIndex)], splatIndex !== this.params.length - 1 ? callArgs.push(new Op('-', new Literal(argLengthVar), new Literal('' + (this.params.length - splatIndex - 1)))) : void 0, new Assign(new Value(splatName), new Call(new Value(new Literal(utility('slice')), [new Access(new Literal('call'))]), callArgs)))].concat(__slice.call((function() {
              var _len2, _ref5, _ref6, _ref7, _results;
              _ref5 = this.params.slice(splatIndex + 1);
              _results = [];
              for (i = 0, _len2 = _ref5.length; i < _len2; i++) {
                param = _ref5[i];
                if ((_ref6 = param.name) != null ? _ref6["this"] : void 0) {
                  param = param.name.properties[0];
                }
                pName = ((_ref7 = param.reference) != null ? _ref7.name : void 0) || param.base || param.name;
                if (param === splat) {
                  continue;
                }
                idx = new Op('-', new Literal(argLengthVar), new Literal(this.params.length - splatIndex - i - 1));
                _results.push(new Assign(new Value(pName), new Value(new Literal('arguments'), [new Index(idx)])));
              }
              return _results;
            }).call(this))))
        ]);
      ...
      for (i = 0, _len2 = vars.length; i < _len2; i++) {
        ...
        if (v instanceof Splat) {
          v = new Param(v, null, true);
          v.splat = true;
        }
        vc = (v.compile(o)).replace(/[\r\n]+\s*/g, '');
        if (!v.splat) {
          o.scope.parameter(vc);
        }
        if ((_ref6 = v.base) != null ? _ref6.asKey : void 0) {
          vc = "" + (v.splat ? '@' : '/*@*/') + vc;
        }
        if (v.value) {
          ...
          if ((pos = vvc.indexOf('*/')) >= 0) {
            vvc = "" + vvc.slice(0, (pos + 1) || 9e9) + "...";
          }
          vc = "" + vc + ("/* = " + vvc + "*/");
        }
        if (v.splat) {
        ...
        if (!(i === vars.length - 1 || (i === vars.length - 2 && (vars[vars.length - 1].splat || vars[vars.length - 1] instanceof Splat)))) {
          vc += ',';
        }
        if (v.splat) {
          vc = "/*" + vc + "*/";
  ...
  exports.Switch = Switch = (function() {
    ...
    Switch.prototype.jumps = function(o/* = {block: true}*/) {
