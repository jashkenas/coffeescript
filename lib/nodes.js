(function() {
  ...
  exports.Code = Code = (function() {
    ...
    Code.prototype.compileNode = function(o) {
      var allNames, code, exprs, formalParams, fp, i, idt, lit, p, param, paramNames, pname, ref, splats, uniqs, val, wasEmpty, _i, _j, _k, _l, _len, _len2, _len3, _len4, _len5, _len6, _len7, _m, _n, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7;
      ...
      for (_k = 0, _len3 = _ref4.length; _k < _len3; _k++) {
        param = _ref4[_k];
        _ref5 = allNames = param.simpleNames();
        for (_l = 0, _len4 = _ref5.length; _l < _len4; _l++) {
          pname = _ref5[_l];
          if (!o.scope.check(pname)) {
            o.scope.parameter(pname);
          }
        }
        paramNames.unshift.apply(paramNames, allNames);
      }
      _ref6 = this.params;
      for (_m = 0, _len5 = _ref6.length; _m < _len5; _m++) {
        ...
        if (param.isComplex()) {
          ...
          if (param.refName != null) {
            paramNames.push(param.refName);
          }
          if (param.value) {
      ...
      for (i = 0, _len6 = formalParams.length; i < _len6; i++) {
        fp = formalParams[i];
        o.scope.parameter(formalParams[i] = fp.compile(o));
      }
      uniqs = {};
      for (_n = 0, _len7 = paramNames.length; _n < _len7; _n++) {
        pname = paramNames[_n];
  ...
  exports.Param = Param = (function() {
    ...
    Param.prototype.asReference = function(o) {
      ...
      if (node["this"]) {
        ...
        if (node.value.reserved) {
          node = new Literal((this.refName = o.scope.freeVariable(node.value)));
        }
      } else if (node.isComplex()) {
        node = new Literal((this.refName = o.scope.freeVariable('arg')));
      }
      node = new Value(node);
      if (this.splat) {
        node = new Splat(node);
    ...
    Param.prototype.simpleNames = function(node) {
      var names, obj, _i, _len, _ref2;
      if (node == null) {
        node = this.name;
      }
      names = [];
      if (!node.objects) {
        if (node["this"] && node.properties[0].name.value.reserved) {
          return [];
        }
        if (node["this"]) {
          return [node.properties[0].name.value];
        }
        return [node.value];
      }
      _ref2 = node.objects;
      for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
        ...
        if (obj instanceof Value) {
          if (obj["this"] && obj.properties[0].name.value.reserved) {
            continue;
          }
          if (obj.isArray() || obj.isObject()) {
            names.unshift.apply(names, this.simpleNames(obj.base));
          } else if (obj["this"]) {
            names.push(obj.properties[0].name.value);
          } else {
